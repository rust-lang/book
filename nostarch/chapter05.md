...
`email` and `username`, as shown in Listing 5-5.
...
A `build_user` function that uses field init shorthand because the `email` and
...
it moves the data, just as we saw in “Ways Variables and Data Interact: Move”
...
We can see the first bit of output came from line 10 in *src/main.rs*, where
we’re debugging the expression `30 * scale`, and its resultant value is 60 (the
