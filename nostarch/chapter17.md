...
you’ll need, primarily from the `futures` at *https://crates.io/crates/futures* and `tokio` at *https://tokio.rs*
crates.
...
remind you of our discussion of iterators back in Chapter 13 at *ch13-02-iterators.html*.
...
* It uses the `impl Trait` syntax we discussed back in the “Traits as
  Parameters” at *ch10-02-traits.html#traits-as-parameters* section in Chapter 10.
...
Listing 17-3: Calling the <code>page_title</code> function from <code>main</code> with a user-supplied argument
...
Listing 17-4: Awaiting an async block with <code>trpl::run</code>
...
Listing 17-6: Using <code>spawn_task</code> to count with two
...
Listing 17-7: Using <code>await</code> with a join handle to run a task to completion
...
Listing 17-8: Using <code>trpl::join</code> to await two anonymous futures
...
Listing 17-9: Creating an async channel and assigning the two halves to <code>tx</code> and <code>rx</code>
...
Listing 17-10: Sending and receiving multiple messages over the async channel and sleeping with an <code>await</code> between each message
...
Listing 17-11: Separating <code>send</code> and <code>recv</code> into their own <code>async</code> blocks and awaiting the futures for those blocks
...
Listing 17-14: Using <code>join!</code> to wait for multiple futures
...
Listing 17-15: Storing anonymous futures in a vector and calling <code>join_all</code>
...
Errors from the run function” at *ch12-03-improving-error-handling-and-modularity.html* in Chapter 12. (We’ll cover trait objects
...
Listing 17-16: Trying to use <code>Box::new</code> to align the types of the futures in a <code>Vec</code>
...
Listing 17-18: Using <code>Pin</code> and <code>Box::pin</code> to make the <code>Vec</code> type check
...
Listing 17-19: Using <code>Pin</code> directly with the <code>pin!</code> macro to avoid unnecessary heap allocations
...
Unlike when we used `race` back in Our First Async Program at *ch17-01-futures-and-syntax.html#our-first-async-program*, we
...
Listing 17-21: Using <code>race</code> to get the result of whichever future finishes first
...
Recall from Our First Async Program at *ch17-01-futures-and-syntax.html#our-first-async-program* that at each await point,
...
Listing 17-22: Using <code>thread::sleep</code> to simulate slow operations
...
Listing 17-23: Using <code>thread::sleep</code> to simulate slow operations
...
Listing 17-24: Using <code>sleep</code> to let operations switch off making progress
...
Listing 17-25: Using <code>yield_now</code> to let operations switch off making progress
...
Listing 17-26: Comparing the performance of <code>sleep</code> and <code>yield_now</code>
...
Listing 17-27: Using our imagined <code>timeout</code> to run a slow operation with a time limit
...
Listing 17-28: Defining the signature of <code>timeout</code>
...
Listing 17-29: Defining <code>timeout</code> with <code>race</code> and <code>sleep</code>
...
async channel in the “Message Passing” at *ch17-02-concurrency-with-async.html#message-passing* earlier in the chapter.
...
Listing 17-32: Filtering a <code>Stream</code> with the <code>StreamExt::filter</code> method
...
Listing 17-33: Using the <code>rx</code> receiver as a <code>ReceiverStream</code>
...
Listing 17-34: Using the <code>StreamExt::timeout</code> method to set a time limit on the items in a stream
...
Listing 17-35: Sending messages through <code>tx</code> with an async delay without making <code>get_messages</code> an async function
...
function, because then we’d return a `Future<Output = Stream<Item = String>>` instead of just a `Stream<Item = String>>`. The caller would have to
...
Listing 17-38: Aligning the types of the the <code>intervals</code> stream with the type of the <code>messages</code> stream
...
Listing 17-39: Using <code>throttle</code> and <code>take</code> to manage the merged streams
...
Back in Futures and the Async Syntax at *ch17-01-futures-and-syntax.html*, we noted that `Future`
...
Recall our description (in the Counting at *ch17-02-concurrency-with-async.html* section) of waiting on
...
In Our First Async Program at *ch17-01-futures-and-syntax.html#our-first-async-program*, we described how a series of await
...
idea of how to fix the code!
 ...
 > detail, the official *Asynchronous Programming in Rust* at *https://rust-lang.github.io/async-book/* book has
 > you covered:
 > 
 > * Chapter 2: Under the Hood: Executing Futures and Tasks at *https://rust-lang.github.io/async-book/02_execution/01_chapter.html*
 > * Chapter 4: Pinning at *https://rust-lang.github.io/async-book/04_pinning/01_chapter.html*
...
Listing 17-41: Using the <code>std::thread</code> APIs instead of the async <code>trpl</code> APIs for the <code>get_intervals</code> function
...
“Building Our Own Async Abstractions” at *ch17-03-more-futures.html#building-our-own-async-abstractions*, or the `throttle`
method we used with streams in “Composing Streams” at *ch17-04-streams.html#composing-streams*. The fact that
