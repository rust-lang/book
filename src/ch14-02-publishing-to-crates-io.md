...
### إنشاء تعليقات توثيق (Documentation) مفيدة

توثيق (documentation) حزمك بدقة سيساعد المستخدمين الآخرين على معرفة كيفية ومتى
...
نمط تعليق التوثيق `//!` يضيف التوثيق إلى العنصر الذي _يحتوي_
التعليقات بدلاً من العناصر _التي تلي_ التعليقات. نستخدم عادةً
...
عن هيكلك الخاص باستخدام `pub use`. _إعادة التصدير_ (Re-exporting) تأخذ عنصرًا عامًا
...
### النشر (Publish) على Crates.io

الآن بعد أن أنشأت حسابًا، وحفظت رمز API الخاص بك، واخترت اسمًا لـ
صندوقك، وحددت البيانات الوصفية المطلوبة، أنت مستعد للنشر (publish)!
