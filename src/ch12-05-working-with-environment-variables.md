...
لحالة الأحرف يمكن للمستخدم تشغيله عبر متغير بيئي. يمكننا جعل هذه الميزة خيارًا
في سطر الأوامر ونطلب من المستخدمين إدخاله في كل مرة يريدون تطبيقه، ولكن بجعله
بدلاً من ذلك متغير بيئي، نسمح لمستخدمينا بتعيين متغير البيئة مرة واحدة ويكون كل
...
ونمررها اسم متغير البيئة `IGNORE_CASE`. تُرجع دالة `env::var` `Result` التي
ستكون متغير `Ok` الناجح الذي يحتوي على قيمة متغير البيئة إذا تم تعيين متغير
البيئة لأي قيمة. ستُرجع متغير `Err` إذا لم يتم تعيين متغير البيئة.

نستخدم تابع `is_ok` على `Result` للتحقق مما إذا كان متغير البيئة مُعيّنًا، مما
يعني أنه يجب على البرنامج إجراء بحث غير حساس لحالة الأحرف. إذا لم يتم تعيين
متغير البيئة `IGNORE_CASE` لأي شيء، فسيُرجع `is_ok` القيمة `false` وسيجري
البرنامج بحثًا حساسًا لحالة الأحرف. نحن لا نهتم بـ _قيمة_ متغير البيئة، فقط ما
...
تسمح بعض البرامج بالمعاملات _ومتغيرات البيئة_ لنفس التكوين. في تلك الحالات،
تقرر البرامج أن أحدهما أو الآخر له الأولوية. لممارسة أخرى بنفسك، حاول التحكم في
الحساسية لحالة الأحرف إما من خلال معامل سطر أوامر أو متغير بيئي. حدد ما إذا كان
يجب أن يكون لمعامل سطر الأوامر أو متغير البيئة الأولوية إذا تم تشغيل البرنامج
...
البيئة: راجع وثائقها لمعرفة ما هو متاح.
