...
التي تطابق نمطاً (pattern) واحداً بينما تتجاهل الباقي. فكر في البرنامج في القائمة 6-6
الذي يطابق قيمة `Option<u8>` في متغير `config_max` ولكنه يريد فقط تنفيذ الكود
إذا كانت القيمة هي متغير (variant) `Some`.
...
إذا كانت القيمة `Some`، نطبع القيمة في متغير (variant) `Some` عن طريق ربط القيمة بالمتغير
`max` في النمط (pattern). لا نريد فعل أي شيء مع قيمة `None`. لإرضاء تعبير `match`، يجب
أن نضيف `_ => ()` بعد معالجة متغير (variant) واحد فقط، وهو كود نموذجي مزعج للإضافة.
...
تأخذ صياغة `if let` نمطاً (pattern) وتعبيراً مفصولين بعلامة يساوي. تعمل بنفس طريقة
`match`، حيث يُعطى التعبير إلى `match` والنمط (pattern) هو ذراعها (arm) الأول. في هذه الحالة،
النمط (pattern) هو `Some(max)`، و `max` يرتبط بالقيمة داخل `Some`. يمكننا حينها استخدام
`max` في جسم كتلة `if let` بنفس الطريقة التي استخدمنا بها `max` في ذراع (arm)
`match` المقابل. الكود في كتلة `if let` يعمل فقط إذا طابقت القيمة النمط (pattern).
...
تشغل الكود عندما تطابق القيمة نمطاً (pattern) واحداً ثم تتجاهل جميع القيم الأخرى.
...
تذكر تعريف تعداد (enum) `Coin` في القائمة 6-4، حيث احتوى متغير (variant) `Quarter` أيضاً على
...
ثم قد نستخدم `if let` لمطابقة (matching) نوع العملة، مقدمين متغير `state` داخل جسم
...
لجعل هذا النمط (pattern) الشائع أفضل للتعبير عنه، تمتلك Rust `let...else`. صياغة
`let...else` تأخذ نمطاً (pattern) على الجانب الأيسر وتعبيراً على اليمين، مشابه جداً لـ
`if let`، لكنها لا تمتلك فرع `if`، فقط فرع `else`. إذا طابق النمط (pattern)، سيربط
القيمة من النمط (pattern) في النطاق الخارجي. إذا _لم_ يطابق النمط (pattern)، سيتدفق البرنامج إلى
ذراع (arm) `else`، والذي يجب أن يعود من الدالة.
...
لقد غطينا الآن كيفية استخدام التعدادات (enums) لإنشاء أنواع مخصصة يمكن أن تكون واحدة
...
التعداد (enum) على بيانات بداخلها، يمكنك استخدام `match` أو `if let` لاستخراج
...
والتعدادات (enums). إنشاء أنواع مخصصة للاستخدام في واجهة برمجة التطبيقات الخاصة بك
