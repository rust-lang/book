...
[Chapter 18][ch19-00-patterns]<!-- ignore --> covers all the different kinds of
patterns and what they do. The power of `match` comes from the expressiveness of
the patterns and the fact that the compiler confirms that all possible cases are
handled.
...
`match` keyword followed by an expression, which in this case is the value
...
which we’ve done in Listing 6-4.
...
then use that binding in the `println!` expression, thus getting the inner state
value out of the `Coin` enum variant for `Quarter`.
...
inside, and then execute code based on it. It’s a bit tricky at first, but once
you get used to it, you’ll wish you had it in all languages. It’s consistently a
user favorite.
...
[“The Tuple Type”][tuples]<!-- ignore --> section) as the code that goes with
the `_` arm:
...
[Chapter 19][ch19-00-patterns]<!-- ignore -->. For now, we’re going to move on
to the `if let` syntax, which can be useful in situations where the `match`
