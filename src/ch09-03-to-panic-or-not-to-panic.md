...
الأنسب كتابة كود يحدث فيه ذعر (panic) بدلاً من إرجاع `Result`. دعنا
نستكشف السبب، ثم نناقش الحالات التي لا يستطيع فيها المصرِّف معرفة أن
الفشل مستحيل، ولكنك كإنسان يمكنك ذلك. سيختتم الفصل ببعض
الإرشادات العامة حول كيفية تقرير ما إذا كنت ستستخدم ذعر (panic) في كود المكتبة.

### الأمثلة، والكود النموذجي، والاختبارات

عندما تكتب مثالاً لتوضيح مفهوم ما، فإن تضمين
كود قوي للتعامل مع الأخطاء (error handling) يمكن أن يجعل المثال أقل وضوحاً. في الأمثلة،
من المفهوم أن استدعاء طريقة مثل `unwrap` التي قد تسبب ذعر (panic) يُقصد به أن يكون
عنصر نائب للطريقة التي تريد أن يتعامل بها تطبيقك مع الأخطاء (errors)، والتي يمكن أن
تختلف بناءً على ما يفعله بقية كودك.

بالمثل، فإن طريقتي `unwrap` و `expect` مفيدتان جداً عندما تكون
في مرحلة النماذج الأولية ولست مستعداً بعد لتقرير كيفية التعامل مع الأخطاء (error handling). فهما تتركان
...
### إرشادات للتعامل مع الأخطاء (Error Handling)

من المستحسن أن يحدث ذعر (panic) في كودك عندما يكون من الممكن أن ينتهي كودك
...
الأفضل إرجاع خطأ (error) إذا استطعت حتى يتمكن مستخدم المكتبة من تحديد
...
الشخص الذي يستخدم مكتبتك للخطأ (error) في كودهم حتى يتمكنوا من إصلاحه
...
ومع ذلك، فإن وجود الكثير من فحوصات الأخطاء (error checking) في جميع دوالك سيكون مطولاً
...
تم تصميم ميزات التعامل مع الأخطاء (error handling) في Rust لمساعدتك في كتابة كود أكثر قوة.
