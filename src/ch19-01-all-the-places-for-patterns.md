...
[Listing 19-1](#listing-19-1) shows that it’s also possible to mix and match `if let`, `else
...
The code in [Listing 19-1](#listing-19-1) determines what color to make your background based on
...
`while` loop to run for as long as a pattern continues to match. In [Listing 19-2](#listing-19-2)
...
interacted with it as an iterator using a `for` loop. As [Listing 19-2](#listing-19-2) shows,
...
pattern. For example, in `for x in y`, the `x` is the pattern. [Listing 19-3](#listing-19-3)
...
The code in [Listing 19-3](#listing-19-3) will print the following:
...
To see the pattern-matching aspect of `let` more clearly, consider [Listing 19-4](#listing-19-4), which uses a pattern with `let` to destructure a tuple.
...
example, [Listing 19-5](#listing-19-5) shows an attempt to destructure a tuple with three
...
Function parameters can also be patterns. The code in [Listing 19-6](#listing-19-6), which
...
function’s arguments to the pattern. [Listing 19-7](#listing-19-7) splits the values in a tuple
