...
إذا لم يطابق عدد العناصر في النمط عدد العناصر في الصف، فلن يطابق النوع الإجمالي وسنحصل على خطأ في المصرِّف. على سبيل المثال، تُظهر القائمة 19-2 محاولة لتفكيك صف بثلاثة عناصر إلى متغيرين، وهو ما لن ينجح.
...
محاولة ترجمة هذا الكود تؤدي إلى خطأ النوع هذا:
...
الجانب السلبي لاستخدام تعبيرات `if let` هو أن المصرِّف لا يتحقق من الشمولية، بينما مع تعبيرات `match` يفعل ذلك. إذا حذفنا كتلة `else` الأخيرة وبالتالي فاتنا معالجة بعض الحالات، فلن ينبهنا المصرِّف إلى خطأ المنطق المحتمل.
...
نقوم بتكييف مُكرِّر باستخدام طريقة `enumerate` بحيث ينتج قيمة والفهرس لتلك القيمة، موضوعة في صف. القيمة الأولى المنتجة هي الصف `(0, 'a')`. عندما يتم مطابقة هذه القيمة مع النمط `(index, value)`، سيكون `index` هو `0` و `value` سيكون `'a'`، طباعة السطر الأول من الناتج.
