...
استدعاء `calculate_length`، لأن `String` تم نقلها (move) إلى
`calculate_length`. بدلاً من ذلك، يمكننا توفير مرجع (reference) لقيمة `String`.
المرجع يشبه المؤشر (pointer) من حيث أنه عنوان يمكننا اتباعه للوصول إلى
...
`String`. هذه العلامات & تمثل المراجع (references)، وتسمح لك بالإشارة إلى
قيمة ما دون أخذ ملكيتها (ownership). الشكل 4-6 يوضح هذا المفهوم.
...
النطاق (scope) الذي يكون فيه المتغير `s` صالحاً هو نفسه نطاق أي معامل
...
نسمي عملية إنشاء مرجع _الاستعارة (borrowing)_. كما في الحياة الواقعية، إذا كان
...
بتعديلات صغيرة تستخدم، بدلاً من ذلك، _مرجعاً قابلاً للتغيير (mutable reference)_:
...
قابل للتغيير أكثر من مرة واحدة في نفس الوقت. الاستعارة القابلة للتغيير (mutable borrow) الأولى في `r1` ويجب أن
...
أوه! نحن _أيضاً_ لا يمكننا الحصول على مرجع قابل للتغيير (mutable reference) بينما لدينا مرجع غير قابل للتغيير (immutable reference)
لنفس القيمة.

مستخدمو المرجع الغير قابل للتغيير لا يتوقعون أن تتغير القيمة فجأة
من تحتهم! ومع ذلك، المراجع الغير قابلة للتغيير (immutable references) المتعددة مسموح بها لأن لا
...
هذه النطاقات لا تتداخل، لذلك هذا الكود مسموح به: المصرِّف يمكنه
...
مصرِّف Rust يشير إلى خطأ محتمل مبكراً (في وقت الترجمة بدلاً
...
في اللغات ذات المؤشرات، من السهل إنشاء _مؤشر معلق (dangling pointer)_
خطأً—مؤشر يشير إلى موقع في الذاكرة ربما تم
إعطاؤه لشخص آخر—عن طريق تحرير (free) بعض الذاكرة مع الحفاظ على مؤشر لتلك
الذاكرة. في Rust، على النقيض، يضمن المصرِّف أن المراجع لن
تكون أبداً مراجع معلقة: إذا كان لديك مرجع لبعض البيانات، سوف
يضمن المصرِّف أن البيانات لن تخرج من النطاق قبل
المرجع للبيانات.
...
سيتم تحرير (drop) `s`. لكننا حاولنا إعادة مرجع له. هذا يعني
...
هذا يعمل بدون أي مشاكل. يتم نقل (move) الملكية، ولا شيء يتم
تحريره.
...
بعد ذلك، سننظر إلى نوع مختلف من المراجع: الشرائح (slices).
