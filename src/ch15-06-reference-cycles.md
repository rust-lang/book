...
<span class="caption" id="15-25">Listing 15-25: A cons list definition that holds a
`RefCell<T>` so we can modify what a `Cons` variant is referring to</span>

We’re using another variation of the `List` definition from [Listing 15-5](./ch15-01-box.html#15-5). The
...
In [Listing 15-26](#15-26), we’re adding a `main` function that uses the definitions in
[Listing 15-25](#15-25). This code creates a list in `a` and a list in `b` that points to
...
<span class="caption" id="15-26">Listing 15-26: Creating a reference cycle of two `List`
...
whether or not a value can be dropped. In [Listing 15-25](#15-25), we always want `Cons`
...
with the value 5 and `leaf` as one of its children, as shown in [Listing 15-27](#15-27):
...
<span class="caption" id="15-27">Listing 15-27: Creating a `leaf` node with no children
...
In [Listing 15-28](#15-28), we update `main` to use this new definition so the `leaf`
...
<span class="caption" id="15-28">Listing 15-28: A `leaf` node with a weak reference to its
parent node `branch`</span>

Creating the `leaf` node looks similar to [Listing 15-27](#15-27) with the exception of
...
[Listing 15-26](#15-26); the `Weak<Node>` references are printed as `(Weak)`:
...
<span class="caption" id="15-29">Listing 15-29: Creating `branch` in an inner scope and
