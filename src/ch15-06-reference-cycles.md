...
We’re using another variation of the `List` definition from [Listing 15-5](ch15-01-box.md#listing-15-5). The
second element in the `Cons` variant is now `RefCell<Rc<List>>`, meaning that
instead of having the ability to modify the `i32` value as we did in [Listing 15-24](ch15-05-interior-mutability.md#listing-15-24), we want to modify the `List` value a `Cons` variant is pointing to.
