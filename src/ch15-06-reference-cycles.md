## الدورات المرجعية (Reference Cycles) يمكن أن تسرب الذاكرة

ضمانات أمان الذاكرة في Rust تجعل من الصعب، ولكن ليس من المستحيل، إنشاء ذاكرة لا يتم تنظيفها أبدًا (المعروفة باسم _تسرب الذاكرة_ memory leak). منع تسرب الذاكرة (memory leak) بالكامل ليس أحد ضمانات Rust، مما يعني أن تسربات الذاكرة (memory leaks) آمنة للذاكرة في Rust. يمكننا أن نرى أن Rust تسمح بتسرب الذاكرة (memory leak) باستخدام `Rc<T>` و `RefCell<T>`: من الممكن إنشاء مراجع حيث تشير العناصر إلى بعضها البعض في دورة (cycle). هذا ينشئ تسربات ذاكرة (memory leaks) لأن عدد المراجع (reference count) لكل عنصر في الدورة (cycle) لن يصل أبدًا إلى 0، ولن يتم إسقاط (drop) القيم أبدًا.

### إنشاء دورة مرجعية (Reference Cycle)

دعنا ننظر في كيف يمكن أن تحدث دورة مرجعية (reference cycle) وكيفية منعها، بدءًا من تعريف enum `List` ووسيلة `tail` في القائمة 15-25.
...
<Listing number="15-26" file-name="src/main.rs" caption="إنشاء دورة مرجعية (reference cycle) من قيمتي `List` تشيران إلى بعضهما البعض">
...
عدد المراجع (reference count) لنسخ `Rc<List>` في كل من `a` و `b` هو 2 بعد أن نغير القائمة في `a` لتشير إلى `b`. في نهاية `main`، تسقط (drops) Rust المتغير `b`، مما يقلل عدد المراجع (reference count) لنسخة `b` `Rc<List>` من 2 إلى 1. لن يتم إسقاط (drop) الذاكرة التي لدى `Rc<List>` على الكومة في هذه النقطة لأن عدد مراجعها (reference count) هو 1، وليس 0. بعد ذلك، تسقط (drops) Rust `a`، مما يقلل عدد المراجع (reference count) لنسخة `a` `Rc<List>` من 2 إلى 1 أيضًا. لا يمكن إسقاط (drop) ذاكرة هذه النسخة أيضًا، لأن نسخة `Rc<List>` الأخرى لا تزال تشير إليها. ستبقى الذاكرة المخصصة للقائمة غير مجموعة إلى الأبد. لتصور هذه الدورة المرجعية (reference cycle)، أنشأنا المخطط في الشكل 15-4.
...
<span class="caption">الشكل 15-4: دورة مرجعية (reference cycle) من القوائم `a` و `b` تشير إلى بعضها البعض</span>

إذا ألغيت تعليق آخر `println!` وشغلت البرنامج، ستحاول Rust طباعة هذه الدورة (cycle) مع `a` تشير إلى `b` تشير إلى `a` وهكذا حتى تتدفق المكدس.

بالمقارنة مع برنامج في العالم الحقيقي، عواقب إنشاء دورة مرجعية (reference cycle) في هذا المثال ليست خطيرة جدًا: مباشرة بعد إنشاء الدورة المرجعية (reference cycle)، ينتهي البرنامج. ومع ذلك، إذا خصص برنامج أكثر تعقيدًا الكثير من الذاكرة في دورة (cycle) واحتفظ بها لفترة طويلة، سيستخدم البرنامج ذاكرة أكثر مما يحتاج وقد يطغى على النظام، مما يتسبب في نفاد الذاكرة المتاحة.

إنشاء دورات مرجعية (reference cycles) ليس سهلاً، ولكنه ليس مستحيلاً أيضًا. إذا كان لديك قيم `RefCell<T>` تحتوي على قيم `Rc<T>` أو مجموعات متداخلة مماثلة من الأنواع ذات القابلية للتغيير الداخلية (interior mutability) وعد المراجع (reference counting)، يجب عليك التأكد من أنك لا تنشئ دورات (cycles)؛ لا يمكنك الاعتماد على Rust لاكتشافها. إنشاء دورة مرجعية (reference cycle) سيكون خطأ منطقيًا في برنامجك يجب عليك استخدام الاختبارات الآلية، ومراجعات الكود، وممارسات تطوير البرمجيات الأخرى لتقليله.

حل آخر لتجنب الدورات المرجعية (reference cycles) هو إعادة تنظيم هياكل البيانات الخاصة بك بحيث تعبر بعض المراجع عن الملكية وبعض المراجع لا تعبر عنها. نتيجة لذلك، يمكن أن يكون لديك دورات (cycles) مكونة من بعض علاقات الملكية وبعض العلاقات غير الملكية، وفقط علاقات الملكية تؤثر على ما إذا كان يمكن إسقاط (drop) قيمة أم لا. في القائمة 15-25، نريد دائمًا أن تمتلك متغيرات `Cons` قائمتها، لذلك إعادة تنظيم هيكل البيانات غير ممكنة. دعنا ننظر في مثال باستخدام الرسوم البيانية المكونة من عقد أبوية وعقد فرعية لمعرفة متى تكون العلاقات غير الملكية طريقة مناسبة لمنع الدورات المرجعية (reference cycles).

<!-- Old headings. Do not remove or links may break. -->

<a id="preventing-reference-cycles-turning-an-rct-into-a-weakt"></a>

### منع الدورات المرجعية (Reference Cycles) باستخدام `Weak<T>`

حتى الآن، أظهرنا أن استدعاء `Rc::clone` يزيد `strong_count` لنسخة `Rc<T>`، ويتم تنظيف نسخة `Rc<T>` فقط إذا كان `strong_count` الخاص بها هو 0. يمكنك أيضًا إنشاء مرجع ضعيف إلى القيمة داخل نسخة `Rc<T>` عن طريق استدعاء `Rc::downgrade` وتمرير مرجع إلى `Rc<T>`. _المراجع القوية_ (strong references) هي كيفية مشاركة ملكية نسخة `Rc<T>`. _المراجع الضعيفة_ (weak references) لا تعبر عن علاقة ملكية، وعددها لا يؤثر على وقت تنظيف نسخة `Rc<T>`. لن تتسبب في دورة مرجعية (reference cycle)، لأن أي دورة (cycle) تتضمن بعض المراجع الضعيفة ستُكسر بمجرد أن يكون عدد المراجع القوية (strong reference count) للقيم المعنية هو 0.

عندما تستدعي `Rc::downgrade`، تحصل على مؤشر ذكي (smart pointer) من نوع `Weak<T>`. بدلاً من زيادة `strong_count` في نسخة `Rc<T>` بمقدار 1، يزيد استدعاء `Rc::downgrade` `weak_count` بمقدار 1. يستخدم نوع `Rc<T>` `weak_count` لتتبع عدد مراجع `Weak<T>` الموجودة، على غرار `strong_count`. الفرق هو أن `weak_count` لا يحتاج إلى أن يكون 0 لكي يتم تنظيف نسخة `Rc<T>`.

لأن القيمة التي يشير إليها `Weak<T>` قد تكون قد أُسقطت (dropped)، للقيام بأي شيء مع القيمة التي يشير إليها `Weak<T>`، يجب عليك التأكد من أن القيمة لا تزال موجودة. قم بذلك عن طريق استدعاء وسيلة `upgrade` على نسخة `Weak<T>`، والتي ستعيد `Option<Rc<T>>`. ستحصل على نتيجة `Some` إذا لم يتم إسقاط (drop) قيمة `Rc<T>` بعد ونتيجة `None` إذا تم إسقاط (drop) قيمة `Rc<T>`. لأن `upgrade` تعيد `Option<Rc<T>>`، ستضمن Rust أنه سيتم التعامل مع حالة `Some` وحالة `None`، ولن يكون هناك مؤشر غير صالح.
...
عدم وجود مخرج لا نهائي يشير إلى أن هذا الكود لم ينشئ دورة مرجعية (reference cycle). يمكننا أيضًا معرفة ذلك من خلال النظر في القيم التي نحصل عليها من استدعاء `Rc::strong_count` و `Rc::weak_count`.
...
عندما ينتهي النطاق الداخلي، تخرج `branch` عن النطاق وينخفض العدد القوي لـ `Rc<Node>` إلى 0، لذلك يتم إسقاط (drop) `Node` الخاص بها. العدد الضعيف 1 من `leaf.parent` ليس له تأثير على ما إذا كان سيتم إسقاط (drop) `Node` أم لا، لذلك لا نحصل على أي تسربات ذاكرة (memory leaks)!
...
كل المنطق الذي يدير الأعداد وإسقاط (drop) القيمة مدمج في `Rc<T>` و `Weak<T>` وتنفيذاتهما لسِمة `Drop`. من خلال تحديد أن العلاقة من طفل إلى والده يجب أن تكون مرجع `Weak<T>` في تعريف `Node`، فإنك قادر على جعل العقد الأبوية تشير إلى العقد الفرعية والعكس بالعكس دون إنشاء دورة مرجعية (reference cycle) وتسربات ذاكرة (memory leaks).

## الملخص

يغطي هذا الفصل كيفية استخدام المؤشرات الذكية (smart pointers) لتقديم ضمانات ومقايضات مختلفة عن تلك التي تقدمها Rust افتراضيًا بالمراجع العادية. نوع `Box<T>` له حجم معروف ويشير إلى البيانات المخصصة على الكومة. يتتبع نوع `Rc<T>` عدد المراجع (reference count) للبيانات على الكومة بحيث يمكن أن يكون للبيانات مالكون متعددون. نوع `RefCell<T>` مع قابليته للتغيير الداخلية (interior mutability) يمنحنا نوعًا يمكننا استخدامه عندما نحتاج إلى نوع غير قابل للتغيير ولكننا بحاجة لتغيير قيمة داخلية لهذا النوع؛ كما يفرض قواعد الاستعارة في وقت التشغيل بدلاً من وقت الترجمة.

تمت مناقشة سِمتي `Deref` و `Drop` أيضًا، اللتان تتيحان الكثير من وظائف المؤشرات الذكية (smart pointers). استكشفنا الدورات المرجعية (reference cycles) التي يمكن أن تتسبب في تسربات ذاكرة (memory leaks) وكيفية منعها باستخدام `Weak<T>`.

إذا أثار هذا الفصل اهتمامك وتريد تنفيذ مؤشراتك الذكية (smart pointers) الخاصة، راجع ["The Rustonomicon"][nomicon] لمزيد من المعلومات المفيدة.

بعد ذلك، سنتحدث عن التزامن في Rust. ستتعلم حتى عن بعض المؤشرات الذكية (smart pointers) الجديدة.
