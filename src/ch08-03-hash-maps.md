...
`split_whitespace` returns iterator to be consumed by `for` loop, which 
iterates over sub-slices separated by whitespace, of the value in `text`.
The `or_insert` method returns a mutable reference (`&mut V`) to the value 
for the specified key. Here we store that mutable reference in the `count`
