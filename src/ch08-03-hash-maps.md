...
`Option<&i32>`, then `unwrap_or` to set `score` to zero if `scores` doesn’t have
...
the hash map, the existing value should remain the way it is; if the key doesn’t
...
it. If it doesn’t, we want to insert the value `50`, and the same for the Blue
