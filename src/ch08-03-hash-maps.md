...
`insert`. In [Listing 8-20](#listing-8-20), we’re keeping track of the scores of two teams whose
...
method, as shown in [Listing 8-21](#listing-8-21).
...
Even though the code in [Listing 8-23](#listing-8-23) calls `insert` twice, the hash map will
...
Blue team. Using the `entry` API, the code looks like [Listing 8-24](#listing-8-24).
...
Running the code in [Listing 8-24](#listing-8-24) will print `{"Yellow": 50, "Blue": 10}`. The
...
update it based on the old value. For instance, [Listing 8-25](#listing-8-25) shows code that
