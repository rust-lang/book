## تخزين المفاتيح مع القيم المرتبطة في خرائط الهاش

آخر مجموعاتنا الشائعة هي خريطة الهاش. النوع `HashMap<K, V>` يخزن تعييناً للمفاتيح من النوع `K` إلى القيم من النوع `V` باستخدام _دالة هاش_، والتي تحدد كيفية وضع هذه المفاتيح والقيم في الذاكرة. العديد من لغات البرمجة تدعم هذا النوع من هياكل البيانات، لكنها غالباً ما تستخدم اسماً مختلفاً، مثل _hash_، _map_، _object_، _hash table_، _dictionary_، أو _associative array_، على سبيل المثال لا الحصر.

خرائط الهاش مفيدة عندما تريد البحث عن البيانات ليس باستخدام فهرس، كما يمكنك مع المتجهات، ولكن باستخدام مفتاح يمكن أن يكون من أي نوع. على سبيل المثال، في لعبة، يمكنك تتبع نقاط كل فريق في خريطة هاش حيث يكون كل مفتاح هو اسم الفريق والقيم هي نقاط كل فريق. بإعطاء اسم فريق، يمكنك استرجاع نقاطه.
...
إحدى الطرق لإنشاء خريطة هاش فارغة هي استخدام `new` وإضافة العناصر باستخدام `insert`. في القائمة 8-20، نتتبع نقاط فريقين اسمهما _Blue_ و _Yellow_. يبدأ فريق Blue بـ 10 نقاط، ويبدأ فريق Yellow بـ 50.

<Listing number="8-20" caption="إنشاء خريطة هاش جديدة وإدراج بعض المفاتيح والقيم">
...
تماماً مثل المتجهات، خرائط الهاش تخزن بياناتها على الكومة. هذه الـ `HashMap` لديها مفاتيح من النوع `String` وقيم من النوع `i32`. مثل المتجهات، خرائط الهاش متجانسة: يجب أن تكون جميع المفاتيح من نفس النوع، ويجب أن تكون جميع القيم من نفس النوع.
...
يمكننا التكرار على كل زوج مفتاح-قيمة في خريطة الهاش بطريقة مشابهة لما نفعله مع المتجهات، باستخدام حلقة `for`:
...
على الرغم من أن عدد أزواج المفاتيح والقيم قابل للنمو، إلا أن كل مفتاح فريد يمكن أن يكون له قيمة واحدة فقط مرتبطة به في كل مرة (لكن ليس العكس: على سبيل المثال، يمكن أن يكون لكل من فريق Blue وفريق Yellow القيمة `10` مخزنة في خريطة الهاش `scores`).
