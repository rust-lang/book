...
## إضافة وظيفة باستخدام التطوير الموجه بالاختبار (Test-Driven Development)

الآن بعد أن أصبح لدينا منطق البحث في _src/lib.rs_ منفصلاً عن دالة `main`، أصبح
من الأسهل بكثير كتابة اختبارات (tests) للوظيفة الأساسية لكودنا. يمكننا استدعاء الدوال
مباشرة بمعاملات (arguments) مختلفة والتحقق من قيم الإرجاع دون الحاجة إلى استدعاء ملفنا
الثنائي من سطر الأوامر (command line).

في هذا القسم، سنضيف منطق البحث إلى برنامج `minigrep` باستخدام عملية التطوير
الموجه بالاختبار (Test-Driven Development / TDD) بالخطوات التالية:

1. اكتب اختبارًا (test) يفشل (fails) وشغّله للتأكد من أنه يفشل (fails) للسبب الذي تتوقعه.
2. اكتب أو عدّل كودًا كافيًا فقط لجعل الاختبار (test) الجديد ينجح (passes).
3. أعد هيكلة الكود الذي أضفته أو غيرته للتو وتأكد من أن الاختبارات (tests) تستمر في
   النجاح (passing).
4. كرر من الخطوة 1!
...
في _src/lib.rs_، سنضيف وحدة `tests` مع دالة اختبار (test function)، كما فعلنا في [الفصل
11][ch11-anatomy]<!-- ignore -->. تحدد دالة الاختبار (test function) السلوك الذي نريد أن تتمتع به
دالة `search`: ستأخذ استعلامًا والنص المراد البحث فيه، وستُرجع فقط الأسطر من
النص التي تحتوي على الاستعلام. توضح القائمة 12-15 هذا الاختبار (test).
