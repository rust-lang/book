...
المهمة الأولى هي جعل `minigrep` يقبل معاملَيْ سطر الأوامر: مسار الملف والنص
المراد البحث عنه. أي، نريد أن نكون قادرين على تشغيل برنامجنا باستخدام `cargo
run`، وشَرْطَتَيْن للإشارة إلى أن المعاملات التالية مخصصة لبرنامجنا وليس لـ
...
لتمكين `minigrep` من قراءة قيم معاملات سطر الأوامر التي نمررها إليه، سنحتاج إلى
دالة `std::env::args` المتوفرة في المكتبة القياسية لـ Rust. تُرجع هذه الدالة
مُكرِّرًا (iterator) لمعاملات سطر الأوامر الممررة إلى `minigrep`. سنغطي
...
<Listing number="12-1" file-name="src/main.rs" caption="جمع معاملات سطر الأوامر في متجه وطباعتها">
