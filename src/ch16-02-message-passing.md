...
First, in [Listing 16-6](#16-6), we’ll create a channel but not do anything with it.
...
<span class="caption" id="16-6">Listing 16-6: Creating a channel and assigning the two
...
[Listing 16-7](#16-7). This is like putting a rubber duck in the river upstream or
...
<span class="caption" id="16-7">Listing 16-7: Moving `tx` to a spawned thread and sending
“hi”</span>
...
In [Listing 16-8](#16-8), we’ll get the value from the receiver in the main thread. This
...
<span class="caption" id="16-8">Listing 16-8: Receiving the value “hi” in the main thread
...
When we run the code in [Listing 16-8](#16-8), we’ll see the value printed from the main
thread:
...
sent it down the channel. Try compiling the code in [Listing 16-9](#16-9) to see why
...
<span class="caption" id="16-9">Listing 16-9: Attempting to use `val` after we’ve sent it
...
us an error if we try to compile the code in [Listing 16-9](#16-9):
...
The code in [Listing 16-8](#16-8) compiled and ran, but it didn’t clearly show us that
two separate threads were talking to each other over the channel. In Listing
16-10 we’ve made some modifications that will prove the code in [Listing 16-8](#16-8) is
...
<span class="caption" id="16-10">Listing 16-10: Sending multiple messages and pausing
between each</span>
...
When running the code in [Listing 16-10](#16-10), you should see the following output
...
single consumer*. Let’s put `mpsc` to use and expand the code in [Listing 16-10](#16-10)
to create multiple threads that all send values to the same receiver. We can do
so by cloning the transmitter, as shown in [Listing 16-11](#16-11):
...
<span class="caption" id="16-11">Listing 16-11: Sending multiple messages from multiple
producers</span>
