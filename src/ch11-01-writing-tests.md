## كيفية كتابة الاختبارات (Writing Tests)

_الاختبارات_ (tests) هي دوال Rust تتحقق من أن الكود غير الاختباري يعمل بالطريقة
المتوقعة. عادةً ما تقوم أجسام دوال الاختبار (test functions) بهذه الإجراءات الثلاثة:
...
تتخذ هذه الإجراءات، والتي تشمل خاصية `test`، وبعض الماكرو، وخاصية
`should_panic`.
...
في أبسط صورها، الاختبار (test) في Rust هو دالة مشروحة بخاصية `test`.
...
إلى دالة اختبار (test function)، أضف `#[test]` في السطر قبل `fn`. عندما تقوم بتشغيل
اختباراتك (tests) باستخدام أمر `cargo test`، تقوم Rust ببناء ملف تنفيذي لتشغيل الاختبارات (test runner) يقوم بتشغيل
الدوال المشروحة ويبلغ عما إذا كانت كل دالة اختبار (test function) تنجح (pass) أو
تفشل (fail).
...
يستخدم جسم الدالة المثال ماكرو `assert_eq!` للتأكد (assertion) من أن `result`،
الذي يحتوي على نتيجة استدعاء `add` بـ 2 و 2، يساوي 4. هذا
التأكيد (assertion) يعمل كمثال على صيغة الاختبار (test) النموذجي. لنقم بتشغيله
لنرى أن هذا الاختبار (test) ينجح (passes).
...
قام Cargo بتجميع وتشغيل الاختبار (test). نرى السطر `running 1 test`. السطر التالي
يعرض اسم دالة الاختبار (test function) المولدة، تسمى `tests::it_works`،
وأن نتيجة تشغيل ذلك الاختبار (test) هي `ok`. الملخص الإجمالي `test
result: ok.` يعني أن جميع الاختبارات (tests) نجحت (passed)، والجزء الذي يقرأ `1
passed; 0 failed` يجمع عدد الاختبارات (tests) التي نجحت (passed) أو فشلت (failed).

من الممكن تحديد اختبار على أنه مُتجاهل بحيث لا يتم تشغيله في حالة معينة
؛ سنغطي ذلك في قسم [تجاهل الاختبارات ما لم يتم طلبها تحديداً][ignoring]<!-- ignore --> لاحقاً في هذا الفصل. لأننا
...
لكن Rust يمكنه ترجمة أي أمثلة كود تظهر في توثيق API الخاص بنا.
تساعد هذه الميزة في الحفاظ على التوثيق والكود متزامنين! سنناقش كيفية
كتابة اختبارات التوثيق في قسم [تعليقات التوثيق كاختبارات][doc-comments]<!-- ignore --> من الفصل 14. في الوقت الحالي، سنتجاهل
إخراج `Doc-tests`.
...
الآن سنضيف اختباراً (test) آخر، لكن هذه المرة سننشئ اختباراً فاشلاً (failing test)! تفشل (fail) الاختبارات (tests)
عندما يحدث panic في دالة الاختبار (test function). يتم تشغيل كل اختبار (test) في خيط جديد
، وعندما يرى الخيط الرئيسي أن خيط اختبار (test thread) قد مات، يتم تحديد الاختبار (test)
على أنه فاشل (failed). في الفصل 9، تحدثنا عن كيف أن أبسط طريقة للـ panic
...
عن طرق تشغيل الاختبارات في قسم [التحكم في كيفية تشغيل الاختبارات][controlling-how-tests-are-run]<!-- ignore -->.
...
بعض الماكرو بخلاف `panic!` التي تكون مفيدة في الاختبارات.
...
اختبار شائع جداً لدرجة أن المكتبة القياسية توفر زوجاً من الماكرو--`assert_eq!`
و`assert_ne!`--لأداء هذا الاختبار بشكل أكثر ملاءمة. تقارن هذه الماكرو قيمتين
...
`assertion failed:`(left == right)``` و``left`ستكون`5`و`right`ستكون`4`.
...
و `!=`، على التوالي. عندما تفشل التأكيدات، تطبع هذه الماكرو معاملاتها
