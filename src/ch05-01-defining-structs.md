...
لتعريف هيكل (struct)، نُدخل الكلمة المفتاحية `struct` ونسمي الهيكل (struct) بأكمله. يجب أن يصف اسم الهيكل (struct) أهمية أجزاء البيانات التي يتم تجميعها معاً. ثم، داخل الأقواس المعقوفة، نحدد أسماء وأنواع أجزاء البيانات، والتي نسميها _الحقول_ (fields). على سبيل المثال، يُظهر Listing 5-1 هيكلاً (struct) يخزن معلومات حول حساب مستخدم.
