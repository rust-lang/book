## استخدام الخيوط (threads) لتشغيل الكود بشكل متزامن (concurrent)

في معظم أنظمة التشغيل الحالية، يتم تشغيل كود البرنامج المنفذ في عملية (process)، وسيدير نظام التشغيل عمليات متعددة في وقت واحد. داخل البرنامج، يمكنك أيضاً أن يكون لديك أجزاء مستقلة تعمل بشكل متزامن (concurrent). الميزات التي تشغل هذه الأجزاء المستقلة تسمى الخيوط (threads). على سبيل المثال، يمكن أن يكون لخادم الويب خيوط متعددة بحيث يمكنه الاستجابة لأكثر من طلب واحد في نفس الوقت.
