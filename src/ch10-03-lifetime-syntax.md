...
فترات الحياة هي نوع آخر من الأنواع العمومية التي كنا نستخدمها بالفعل. بدلاً من ضمان أن النوع له السلوك الذي نريده، تضمن فترات الحياة أن المراجع صالحة طالما أننا بحاجة إليها.
...
يحتوي مصرِّف Rust على _مدقق استعارة_ يقارن النطاقات لتحديد ما إذا كانت جميع الاستعارات صالحة. تُظهر القائمة 10-17 نفس الكود الموجود في القائمة 10-16 ولكن مع تعليقات توضح فترات حياة المتغيرات.
...
تعليقات فترة الحياة لا تغير المدة التي تعيشها أي من المراجع. بل، تصف العلاقات بين فترات حياة المراجع المتعددة دون التأثير على فترات الحياة. تمامًا كما يمكن للدوال قبول أي نوع عندما يحدد التوقيع معامل نوع عمومي، يمكن للدوال قبول المراجع بأي فترة حياة من خلال تحديد معامل فترة حياة عامة.

تعليقات فترة الحياة لها صياغة غير عادية قليلاً: أسماء معاملات فترة الحياة يجب أن تبدأ بعلامة الاقتباس الفردية (`'`) وعادة ما تكون كلها أحرفًا صغيرة وقصيرة جدًا، مثل الأنواع العمومية. معظم الناس يستخدمون الاسم `'a` لأول تعليق لفترة الحياة. نضع تعليقات معامل فترة الحياة بعد `&` من المرجع، باستخدام مسافة لفصل التعليق عن نوع المرجع.
...
بعد كتابة الكثير من كود Rust، وجد فريق Rust أن مبرمجي Rust كانوا يدخلون تعليقات فترة الحياة نفسها مرارًا وتكرارًا في حالات معينة. كانت هذه الحالات متوقعة وتتبع بعض الأنماط الحتمية. قام المطورون ببرمجة هذه الأنماط في كود المصرِّف بحيث يمكن لمدقق الاستعارة استنتاج فترات الحياة في هذه الحالات ولا يحتاج إلى تعليقات صريحة.

يُذكر هذا الجزء من تاريخ Rust لأنه من الممكن أن تظهر المزيد من الأنماط الحتمية وتضاف إلى المصرِّف. في المستقبل، قد تكون هناك حاجة إلى عدد أقل من تعليقات فترة الحياة.

الأنماط المبرمجة في تحليل Rust للمراجع تسمى _قواعد استبعاد فترة الحياة_. هذه ليست قواعد يجب على المبرمجين اتباعها؛ إنها مجموعة من الحالات الخاصة التي سينظر فيها المصرِّف، وإذا كان كودك يتناسب مع هذه الحالات، فلن تحتاج إلى كتابة فترات الحياة بشكل صريح.

قواعد الاستبعاد لا توفر استنتاجًا كاملاً. إذا طبق Rust القواعد بشكل حتمي لكن لا يزال هناك غموض حول ماهية فترات حياة المراجع، فلن يخمن المصرِّف ما يجب أن تكون عليه فترة حياة المراجع المتبقية. بدلاً من التخمين، سيعطيك المصرِّف خطأ يمكنك حله عن طريق إضافة تعليقات فترة الحياة.
...
يستخدم المصرِّف ثلاث قواعد لمعرفة فترات حياة المراجع عندما لا توجد تعليقات صريحة. تنطبق القاعدة الأولى على فترات حياة الإدخال، وتنطبق القاعدتان الثانية والثالثة على فترات حياة الإخراج. إذا وصل المصرِّف إلى نهاية القواعد الثلاث ولا يزال هناك مراجع لا يمكنه معرفة فترات حياتها، سيتوقف المصرِّف مع خطأ. تنطبق هذه القواعد على تعريفات `fn` وكذلك كتل `impl`.

القاعدة الأولى هي أن المصرِّف يعين معامل فترة حياة لكل معامل هو مرجع. بعبارة أخرى، دالة بمعامل واحد تحصل على معامل فترة حياة واحد: `fn foo<'a>(x: &'a i32)`؛ دالة بمعاملين تحصل على معاملي فترة حياة منفصلين: `fn foo<'a, 'b>(x: &'a i32, y: &'b i32)`؛ وهكذا.
