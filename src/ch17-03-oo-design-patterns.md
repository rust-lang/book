...
[Listing 17-11](#17-11) shows this workflow in code form: this is an example usage of the
...
<span class="caption" id="17-11">Listing 17-11: Code that demonstrates the desired
...
instance of `Post`, as shown in [Listing 17-12](#17-12). We’ll also make a private
...
<span class="caption" id="17-12">Listing 17-12: Definition of a `Post` struct and a `new`
...
We saw in [Listing 17-11](#17-11) that we want to be able to call a method named
...
straightforward, so let’s add the implementation in [Listing 17-13](#17-13) to the `impl
Post` block:
...
<span class="caption" id="17-13">Listing 17-13: Implementing the `add_text` method to add
...
still in the draft state, as shown on line 7 of [Listing 17-11](#17-11). For now, let’s
implement the `content` method with the simplest thing that will fulfill this
requirement: always returning an empty string slice. We’ll change this later
once we implement the ability to change a post’s state so it can be published.
So far, posts can only be in the draft state, so the post content should always
be empty. [Listing 17-14](#17-14) shows this placeholder implementation:
...
<span class="caption" id="17-14">Listing 17-14: Adding a placeholder implementation for
the `content` method on `Post` that always returns an empty string slice</span>

With this added `content` method, everything in [Listing 17-11](#17-11) up to line 7
works as intended.

### Requesting a Review of the Post Changes Its State

Next, we need to add functionality to request a review of a post, which should
change its state from `Draft` to `PendingReview`. [Listing 17-15](#17-15) shows this code:
...
<span class="caption" id="17-15">Listing 17-15: Implementing `request_review` methods on
...
[Listing 17-11](#17-11) now works up to line 10!
...
state is approved, as shown in [Listing 17-16](#17-16):
...
<span class="caption" id="17-16">Listing 17-16: Implementing the `approve` method on
`Post` and the `State` trait</span>
...
as shown in [Listing 17-17](#17-17):
...
<span class="caption" id="17-17">Listing 17-17: Updating the `content` method on `Post` to
...
have, as shown in [Listing 17-18](#17-18):
...
<span class="caption" id="17-18">Listing 17-18: Adding the `content` method to the `State`
trait</span>
...
And we’re done—all of [Listing 17-11](#17-11) now works! We’ve implemented the state
...
Let’s consider the first part of `main` in [Listing 17-11](#17-11):
...
[Listing 17-19](#17-19) shows the definition of a `Post` struct and a `DraftPost` struct,
...
<span class="caption" id="17-19">Listing 17-19: A `Post` with a `content` method and a
...
<span class="caption" id="17-20">Listing 17-20: A `PendingReviewPost` that gets created by
...
The updated code in `main` is shown in [Listing 17-21](#17-21):
...
<span class="caption" id="17-21">Listing 17-21: Modifications to `main` to use the new
...
is after [Listing 17-21](#17-21) to see what you think about the design of this version
