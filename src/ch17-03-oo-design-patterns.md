...
Using `std::mem::take()` method is necessary because Rust won't let us have unpopulated fields in structs.  The `take` method moves the `state` value out of `Post` rather than borrowing it. 

This is where the `Option` in the `state` field of `Post` comes in: we call the `take()` method to transfer `Some` value out of the `state` field and leave a `None` in its place temporarily. 
