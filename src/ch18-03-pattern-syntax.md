...
with all variables. In [Listing 18-11](#18-11), we declare a variable named `x` with the
...
<span class="caption" id="18-11">Listing 18-11: A `match` expression with an arm that
...
[Listing 18-12](#18-12) shows a `Point` struct with two fields, `x` and `y`, that we can
...
<span class="caption" id="18-12">Listing 18-12: Destructuring a struct’s fields into
...
from the pattern will have the same names. [Listing 18-13](#18-13) behaves in the same
way as the code in [Listing 18-12](#18-12), but the variables created in the `let`
...
<span class="caption" id="18-13">Listing 18-13: Destructuring struct fields using struct
...
In [Listing 18-14](#18-14), we have a `match` expression that separates `Point` values
...
<span class="caption" id="18-14">Listing 18-14: Destructuring and matching literal values
in one pattern</span>
...
We've destructured enums in this book (for example, [Listing 6-5](./ch06-02-match.html#6-5) in Chapter 6),
but haven’t yet explicitly discussed that the pattern to destructure an enum
corresponds to the way the data stored within the enum is defined. As an
example, in [Listing 18-15](#18-15) we use the `Message` enum from [Listing 6-2](./ch06-01-defining-an-enum.html#6-2) and write
...
<span class="caption" id="18-15">Listing 18-15: Destructuring enum variants that hold
...
code in [Listing 18-15](#18-15) to support RGB and HSV colors in the `ChangeColor`
message, as shown in [Listing 18-16](#18-16).

```rust
{{#rustdoc_include ../listings/ch18-patterns-and-matching/listing-18-16/src/main.rs}}
```

<span class="caption" id="18-16">Listing 18-16: Matching on nested enums</span>
...
parameters, as shown in [Listing 18-17](#18-17).
...
<span class="caption" id="18-17">Listing 18-17: Using `_` in a function signature</span>
...
other parts in the corresponding code we want to run. [Listing 18-18](#18-18) shows code
...
<span class="caption" id="18-18">Listing 18-18: Using an underscore within patterns that
...
particular values. [Listing 18-19](#18-19) shows an example of ignoring the second and
...
<span class="caption" id="18-19">Listing 18-19: Ignoring multiple parts of a tuple</span>
...
with an underscore. In [Listing 18-20](#18-20), we create two unused variables, but when
...
<span class="caption" id="18-20">Listing 18-20: Starting a variable name with an
...
distinction matters, [Listing 18-21](#18-21) will provide us with an error.

```rust,ignore,does_not_compile
{{#rustdoc_include ../listings/ch18-patterns-and-matching/listing-18-21/src/main.rs:here}}
```

<span class="caption" id="18-21">Listing 18-21: An unused variable starting with an
underscore still binds the value, which might take ownership of the value</span>

We’ll receive an error because the `s` value will still be moved into `_s`,
which prevents us from using `s` again. However, using the underscore by itself
doesn’t ever bind to the value. [Listing 18-22](#18-22) will compile without any errors
...
<span class="caption" id="18-22">Listing 18-22: Using an underscore does not bind the
value</span>
...
explicitly matched in the rest of the pattern. In [Listing 18-23](#18-23), we have a
...
<span class="caption" id="18-23">Listing 18-23: Ignoring all fields of a `Point` except
for `x` by using `..`</span>
...
The syntax `..` will expand to as many values as it needs to be. [Listing 18-24](#18-24)
...
<span class="caption" id="18-24">Listing 18-24: Matching only the first and last values in
...
[Listing 18-25](#18-25) shows an example of using `..` ambiguously, so it will not
compile.
...
<span class="caption" id="18-25">Listing 18-25: An attempt to use `..` in an ambiguous
way</span>
...
The condition can use variables created in the pattern. [Listing 18-26](#18-26) shows a
...
<span class="caption" id="18-26">Listing 18-26: Adding a match guard to a pattern</span>
...
In [Listing 18-11](#18-11), we mentioned that we could use match guards to solve our
pattern-shadowing problem. Recall that we created a new variable inside the
pattern in the `match` expression instead of using the variable outside the
`match`. That new variable meant we couldn’t test against the value of the
outer variable. [Listing 18-27](#18-27) shows how we can use a match guard to fix this
problem.
...
<span class="caption" id="18-27">Listing 18-27: Using a match guard to test for equality
...
<span class="caption" id="18-28">Listing 18-28: Combining multiple patterns with a match
guard</span>
...
time as we’re testing that value for a pattern match. In [Listing 18-29](#18-29), we want
...
<span class="caption" id="18-29">Listing 18-29: Using `@` to bind to a value in a pattern
