...
نستخدم الأنواع العمومية لإنشاء تعريفات لعناصر مثل توقيعات الدوال أو الهياكل، والتي يمكننا استخدامها بعد ذلك مع العديد من أنواع البيانات الملموسة المختلفة. دعنا أولاً ننظر إلى كيفية تعريف الدوال والهياكل والتعدادات والأساليب باستخدام الأنواع العمومية. بعد ذلك، سنناقش كيف تؤثر الأنواع العمومية على أداء الكود.

### في تعريفات الدوال

عند تعريف دالة تستخدم الأنواع العمومية، نضع الأنواع العمومية في توقيع الدالة حيث نحدد عادةً أنواع البيانات للمعاملات والقيمة المُرجعة. القيام بذلك يجعل كودنا أكثر مرونة ويوفر المزيد من الوظائف للمستدعين لدالتنا مع منع تكرار الكود.
...
دالة `largest_i32` هي التي استخرجناها في القائمة 10-3 والتي تجد أكبر `i32` في شريحة. تجد دالة `largest_char` أكبر `char` في شريحة. تحتوي أجسام الدوال على نفس الكود، لذا دعنا نزيل التكرار من خلال إدخال معامل نوع عمومي في دالة واحدة.
...
يمكننا أيضًا تعريف الهياكل لاستخدام معامل نوع عمومي في حقل واحد أو أكثر باستخدام صيغة `<>`. تُعرّف القائمة 10-6 هيكل `Point<T>` لاحتواء قيم الإحداثيات `x` و `y` من أي نوع.
...
الصيغة لاستخدام الأنواع العمومية في تعريفات الهياكل مشابهة لتلك المستخدمة في تعريفات الدوال. أولاً، نُعلن اسم معامل النوع داخل الأقواس الزاوية مباشرة بعد اسم الهيكل. ثم، نستخدم النوع العام في تعريف الهيكل حيث سنحدد خلاف ذلك أنواع البيانات الملموسة.
...
في هذا المثال، عندما نعين القيمة الصحيحة `5` لـ `x`، نُعلم المصرِّف أن النوع العام `T` سيكون عددًا صحيحًا لهذا المثيل من `Point<T>`. ثم، عندما نحدد `4.0` لـ `y`، والذي عرّفناه ليكون من نفس نوع `x`، سنحصل على خطأ عدم تطابق النوع مثل هذا:
...
الآن جميع مثيلات `Point` الموضحة مسموح بها! يمكنك استخدام العديد من معاملات النوع العام في تعريف كما تريد، ولكن استخدام أكثر من بضعة يجعل كودك صعب القراءة. إذا وجدت أنك بحاجة إلى الكثير من الأنواع العمومية في كودك، فقد يشير ذلك إلى أن كودك بحاجة إلى إعادة هيكلة إلى قطع أصغر.
...
عندما تتعرف على مواقف في كودك مع تعريفات هياكل أو تعدادات متعددة تختلف فقط في أنواع القيم التي تحتوي عليها، يمكنك تجنب التكرار باستخدام الأنواع العمومية بدلاً من ذلك.

### في تعريفات الأساليب

يمكننا تطبيق الأساليب على الهياكل والتعدادات (كما فعلنا في الفصل 5) واستخدام الأنواع العمومية في تعريفاتها أيضًا. تُظهر القائمة 10-9 هيكل `Point<T>` الذي عرّفناه في القائمة 10-6 مع أسلوب يُدعى `x` مُطبق عليه.
...
قد تتساءل عما إذا كانت هناك تكلفة وقت التشغيل عند استخدام معاملات النوع العام. الخبر السار هو أن استخدام الأنواع العمومية لن يجعل برنامجك يعمل بشكل أبطأ مما سيكون عليه مع الأنواع الملموسة.

تحقق Rust ذلك من خلال إجراء التوحيد الشكلي (monomorphization) للكود باستخدام الأنواع العمومية في وقت الترجمة. _التوحيد الشكلي_ هو عملية تحويل الكود العام إلى كود محدد عن طريق ملء الأنواع الملموسة المستخدمة عند الترجمة. في هذه العملية، يقوم المصرِّف بعكس الخطوات التي استخدمناها لإنشاء الدالة العامة في القائمة 10-5: ينظر المصرِّف إلى جميع الأماكن التي يتم فيها استدعاء الكود العام ويُنشئ كودًا للأنواع الملموسة التي يتم استدعاء الكود العام بها.
...
يتم استبدال `Option<T>` العام بالتعريفات المحددة التي أنشأها المصرِّف. لأن Rust تُترجم الكود العام إلى كود يحدد النوع في كل مثيل، لا ندفع أي تكلفة وقت التشغيل لاستخدام الأنواع العمومية. عندما يعمل الكود، يؤدي تمامًا كما لو كنا قد كررنا كل تعريف يدويًا. تجعل عملية التوحيد الشكلي الأنواع العمومية في Rust فعالة للغاية في وقت التشغيل.
