...
يسمح لك أمر `cargo install` بتثبيت واستخدام الصناديق الثنائية (binaries)
...
الحزم التي لديها أهداف ثنائية (binary targets). _الهدف الثنائي_ (binary target) هو البرنامج القابل للتشغيل
الذي يتم إنشاؤه إذا كان لدى الصندوق ملف _src/main.rs_ أو ملف آخر محدد
كملف ثنائي (binary)، على عكس هدف المكتبة الذي لا يمكن تشغيله بمفرده ولكن
مناسب للتضمين داخل برامج أخرى. عادةً، تحتوي الصناديق على
معلومات في ملف README حول ما إذا كان الصندوق مكتبة، أم لديه
هدف ثنائي، أم كلاهما.

جميع الملفات الثنائية (binaries) المثبتة باستخدام `cargo install` يتم تخزينها في مجلد _bin_
