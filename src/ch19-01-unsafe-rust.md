...
[Listing 19-1](#19-1) shows how to create an immutable and a mutable raw pointer from
references.
...
<span class="caption" id="19-1">Listing 19-1: Creating raw pointers from references</span>
...
so certain of. [Listing 19-2](#19-2) shows how to create a raw pointer to an arbitrary
...
<span class="caption" id="19-2">Listing 19-2: Creating a raw pointer to an arbitrary
memory address</span>

Recall that we can create raw pointers in safe code, but we can’t *dereference*
raw pointers and read the data being pointed to. In [Listing 19-3](#19-3), we use the
...
<span class="caption" id="19-3">Listing 19-3: Dereferencing raw pointers within an
`unsafe` block</span>

Creating a pointer does no harm; it’s only when we try to access the value that
it points at that we might end up dealing with an invalid value.

Note also that in [Listing 19-1](#19-1) and 19-3, we created `*const i32` and `*mut i32`
...
argument. [Listing 19-4](#19-4) shows how to use `split_at_mut`.

```rust
{{#rustdoc_include ../listings/ch19-advanced-features/listing-19-04/src/main.rs:here}}
```

<span class="caption" id="19-4">Listing 19-4: Using the safe `split_at_mut`
function</span>

We can’t implement this function using only safe Rust. An attempt might look
something like [Listing 19-5](#19-5), which won’t compile. For simplicity, we’ll
...
<span class="caption" id="19-5">Listing 19-5: An attempted implementation of
...
When we try to compile the code in [Listing 19-5](#19-5), we’ll get an error.
...
[Listing 19-6](#19-6) shows how to use an `unsafe` block, a raw pointer, and some calls
...
<span class="caption" id="19-6">Listing 19-6: Using unsafe code in the implementation of
...
In contrast, the use of `slice::from_raw_parts_mut` in [Listing 19-7](#19-7) would
...
<span class="caption" id="19-7">Listing 19-7: Creating a slice from an arbitrary memory
location</span>
...
[Listing 19-8](#19-8) demonstrates how to set up an integration with the `abs` function
...
<span class="caption" id="19-8">Listing 19-8: Declaring and calling an `extern` function
...
In Rust, global variables are called *static* variables. [Listing 19-9](#19-9) shows an
...
<span class="caption" id="19-9">Listing 19-9: Defining and using an immutable static
variable</span>
...
*unsafe*. [Listing 19-10](#19-10) shows how to declare, access, and modify a mutable
...
<span class="caption" id="19-10">Listing 19-10: Reading from or writing to a mutable
...
<span class="caption" id="19-11">Listing 19-11: Defining and implementing an unsafe
trait</span>
