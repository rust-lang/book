...
union field reads the bits of the union at the field's type. It is the programmer's
...
type `bool` is undefined behavior.
...
Another way to access union fields is to use pattern matching. Since pattern matching
is like reading the union with a particular field, it has to be placed in `unsafe`
blocks as well.
