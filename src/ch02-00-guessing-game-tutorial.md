...
لنبدأ في تعلم Rust من خلال العمل على مشروع عملي (hands-on project) معاً! يقدم لك هذا الفصل بعض المفاهيم (concepts) الشائعة في Rust من خلال توضيح كيفية استخدامها في برنامج حقيقي. ستتعلم عن `let` و `match` والدوال (functions) والدوال المرتبطة (associated functions) والصناديق الخارجية (external crates) والمزيد! في الفصول التالية، سنستكشف هذه الأفكار بمزيد من التفصيل. في هذا الفصل، ستمارس الأساسيات فقط.
...
يحتوي هذا الكود على الكثير من المعلومات، لذا دعنا نستعرضه سطراً بسطر. للحصول على إدخال المستخدم (user input) ثم طباعة النتيجة كمخرج (output)، نحتاج إلى إحضار مكتبة (library) `io` (الإدخال/الإخراج - input/output) إلى النطاق (scope). تأتي مكتبة `io` من المكتبة القياسية (standard library)، المعروفة باسم `std`:

```rust,ignore
{{#rustdoc_include ../listings/ch02-guessing-game-tutorial/listing-02-01/src/main.rs:io}}
```

بشكل افتراضي، يحتوي Rust على مجموعة من العناصر (items) المعرفة في المكتبة القياسية (standard library) التي يجلبها إلى نطاق (scope) كل برنامج. تُسمى هذه المجموعة _التمهيدية (prelude)_، ويمكنك رؤية كل شيء فيها [في توثيق المكتبة القياسية][prelude].

إذا لم يكن النوع (type) الذي تريد استخدامه في التمهيدية (prelude)، فعليك إحضار هذا النوع إلى النطاق (scope) صراحةً باستخدام عبارة (statement) `use`. استخدام مكتبة (library) `std::io` يوفر لك عدداً من الميزات (features) المفيدة، بما في ذلك القدرة على قبول إدخال المستخدم (user input).

كما رأيت في الفصل 1، دالة (function) `main` هي نقطة الدخول (entry point) إلى البرنامج:

```rust,ignore
{{#rustdoc_include ../listings/ch02-guessing-game-tutorial/listing-02-01/src/main.rs:main}}
```

يُعلن صيغة (syntax) `fn` عن دالة (function) جديدة؛ الأقواس (parentheses)، `()`، تشير إلى عدم وجود معاملات (parameters)؛ والقوس المعقوف (curly bracket)، `{`، يبدأ جسم الدالة (function body).

كما تعلمت أيضاً في الفصل 1، `println!` هو ماكرو (macro) يطبع نصاً (text) على الشاشة:
...
بعد ذلك، سننشئ _متغيراً (variable)_ لتخزين إدخال المستخدم (user input)، هكذا:

```rust,ignore
{{#rustdoc_include ../listings/ch02-guessing-game-tutorial/listing-02-01/src/main.rs:string}}
```

الآن أصبح البرنامج مثيراً للاهتمام! يحدث الكثير في هذا السطر الصغير. نستخدم عبارة (statement) `let` لإنشاء المتغير (variable). إليك مثال آخر:
...
يُغطي [الفصل 6][enums]<!-- ignore --> التعدادات بمزيد من التفصيل. الغرض من أنواع `Result` هذه هو ترميز معلومات معالجة الأخطاء.
...
إذا قمت على الفور بتشغيل `cargo build` مرة أخرى دون إجراء أي تغييرات، فلن تحصل على أي مخرج بصرف النظر عن سطر `Finished`. يعلم Cargo أنه قد حمّل وجمع التبعيات بالفعل، ولم تغير أي شيء عنها في ملف _Cargo.toml_. يعلم Cargo أيضاً أنك لم تغير أي شيء عن الكود الخاص بك، لذلك لا يعيد ترجمة ذلك أيضاً. بدون شيء للقيام به، فإنه ببساطة يخرج.
...
هناك الكثير لقوله عن [Cargo][doccargo]<!-- ignore --> [ونظامه البيئي][doccratesio]<!-- ignore --> والذي سنناقشه في الفصل 14، لكن في الوقت الحالي، هذا كل ما تحتاج إلى معرفته. يسهل Cargo إعادة استخدام المكتبات كثيراً، لذلك يستطيع Rustaceans كتابة مشاريع أصغر يتم ترجمتها من عدد من الحزم.
