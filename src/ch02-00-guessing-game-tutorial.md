...
러스트는 모든 프로그램의 스코프에 [the *prelude*][prelude]<!-- ignore -->
...
자세한 내용은 3장의
[“Variables and Mutability”][variables-and-mutability]<!-- ignore -->
...
[`std::io::Stdin`][iostdin]<!-- ignore -->의 인스턴스를 돌려줍니다.
...
여기서 돌려준 값은 [`io::Result`][ioresult]<!-- ignore -->
...
`Result` 타입은 [*열거형 (enumerations)*][enums]<!-- ignore --> 으로써
...
`io::Result` 인스턴스는 [`expect` 메소드][expect]<!-- ignore -->를
