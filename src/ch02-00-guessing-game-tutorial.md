...
$cargo build     
  ...
  Updating libc v0.2.153 -> v0.2.177
 ...
 Compiling listing-02-02 v0.1.0 (file:///projects/guessing_game)
...
released newer 0.9.x patch releases, running `cargo update` will pick the
latest compatible patch.

<!-- manual-regeneration
cd listings/ch02-guessing-game-tutorial/listing-02-02/
cargo update
assuming there is a new 0.9.x version of rand; otherwise use another update
...
$ cargo update
  ...
  Updating rand v0.9.2 -> v0.9.3 (available: v0.999.0)
```
...
you are now using is 0.9.3. To use a hypothetical 0.999.x series, you’d have
to update the _Cargo.toml_ file to look like this instead (don’t actually make
this change because the following examples assume you’re using `rand` 0.9.2):
...
First, add the line `use rand::random_range;`. The `random_range` convenience
function provided by `rand` (v0.9.x) accepts a range expression and returns a
random value within that range. For example:
...
Here `random_range(1..=100)` generates a random number in the inclusive range
from 1 to 100. This API avoids needing to bring a trait like `Rng` into scope
or calling `thread_rng()` directly for this simple use case.
