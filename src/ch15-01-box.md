...
المؤشر الذكي (smart pointer) الأكثر وضوحًا هو الصندوق (box)، والذي يُكتب نوعه `Box<T>`. _الصناديق_ (boxes) تسمح لك بتخزين البيانات على الكومة (heap) بدلاً من المكدس (stack). ما يبقى على المكدس هو المؤشر إلى بيانات الكومة. ارجع إلى الفصل 4 لمراجعة الفرق بين المكدس والكومة.
...
نوع `Box<T>` هو مؤشر ذكي (smart pointer) لأنه ينفذ سِمة `Deref`، والتي تسمح لقيم `Box<T>` بالتعامل معها كمراجع. عندما تخرج قيمة `Box<T>` عن النطاق، يتم تنظيف بيانات الكومة التي يشير إليها الصندوق أيضًا بسبب تنفيذ سِمة `Drop`. ستكون هاتان السمتان أكثر أهمية للوظائف التي توفرها أنواع المؤشرات الذكية (smart pointers) الأخرى التي سنناقشها في بقية هذا الفصل. دعنا نستكشف هاتين السمتين بمزيد من التفصيل.
