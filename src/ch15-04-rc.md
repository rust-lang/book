...
Let’s return to our cons list example in [Listing 15-5](./ch15-01-box.html#15-5). Recall that we defined
...
won’t work, as shown in [Listing 15-17](#15-17):
...
<span class="caption" id="15-17">Listing 15-17: Demonstrating we’re not allowed to have
...
in [Listing 15-17](#15-17), but not in every scenario.

Instead, we’ll change our definition of `List` to use `Rc<T>` in place of
`Box<T>`, as shown in [Listing 15-18](#15-18). Each `Cons` variant will now hold a value
...
<span class="caption" id="15-18">Listing 15-18: A definition of `List` that uses
`Rc<T>`</span>
...
Let’s change our working example in [Listing 15-18](#15-18) so we can see the reference
counts changing as we create and drop references to the `Rc<List>` in `a`.

In [Listing 15-19](#15-19), we’ll change `main` so it has an inner scope around list `c`;
...
<span class="caption" id="15-19">Listing 15-19: Printing the reference count</span>
