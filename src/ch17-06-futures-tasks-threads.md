## الجمع بين كل شيء: المستقبلات (Futures) والمهام (Tasks) والخيوط (Threads)

كما رأينا في [الفصل 16][ch16]<!-- ignore -->، توفر الخيوط (threads) نهجًا واحدًا للتزامن (concurrency). لقد رأينا نهجًا آخر في هذا الفصل: استخدام async مع المستقبلات (futures) والتدفقات (streams). إذا كنت تتساءل متى تختار إحدى الطريقتين على الأخرى، فالإجابة هي: يعتمد! وفي كثير من الحالات، لا يكون الخيار هو الخيوط _أو_ async بل الخيوط _و_ async.
...
هناك سبب لكون واجهات برمجة التطبيقات لإنشاء الخيوط (threads) وإنشاء المهام (tasks) متشابهة جدًا. الخيوط (threads) تعمل كحدود لمجموعات من العمليات المتزامنة (concurrent)؛ التزامن (concurrency) ممكن _بين_ الخيوط (threads). المهام (tasks) تعمل كحدود لمجموعات من العمليات _غير المتزامنة_ (asynchronous)؛ التزامن (concurrency) ممكن _بين_ و _داخل_ المهام (tasks)، لأن المهمة (task) يمكنها التبديل بين المستقبلات (futures) في نصها. أخيرًا، المستقبلات (futures) هي أصغر وحدة تزامن (concurrency) في Rust، وكل مستقبل (future) قد يمثل شجرة من المستقبلات (futures) الأخرى. بيئة التشغيل (runtime) - على وجه الخصوص، منفذها (executor) - تدير المهام (tasks)، والمهام (tasks) تدير المستقبلات (futures). من هذا المنظور، المهام تشبه الخيوط الخفيفة المدارة من بيئة التشغيل مع قدرات إضافية تأتي من كونها مدارة من بيئة تشغيل بدلاً من نظام التشغيل.
