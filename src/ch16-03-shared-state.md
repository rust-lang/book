...
تمرير الرسائل (message passing) هو طريقة جيدة للتعامل مع التزامن (concurrency)، لكنها ليست الطريقة الوحيدة. هناك طريقة أخرى وهي أن تصل خيوط (threads) متعددة إلى نفس البيانات المشتركة (shared data). فكر في هذا الجزء من شعار توثيق لغة Go مرة أخرى: "لا تتواصل بمشاركة الذاكرة."
...
Mutex هو اختصار للاستبعاد المتبادل (mutual exclusion)، كما في أن mutex يسمح فقط لخيط (thread) واحد بالوصول إلى بعض البيانات في أي وقت معين. للوصول إلى البيانات في mutex، يجب على الخيط (thread) أولاً أن يشير إلى أنه يريد الوصول عن طريق طلب الحصول على قفل (lock) الـ mutex. القفل (lock) هو بنية بيانات تعد جزءاً من الـ mutex تتتبع من لديه حالياً وصول حصري إلى البيانات. لذلك، يوصف الـ mutex بأنه يحرس (guarding) البيانات التي يحملها عبر نظام القفل (lock).
