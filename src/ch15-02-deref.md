...
reference; the dereference operator used on the `Box<T>` in [Listing 15-7](#listing-15-7)
...
The main difference between [Listing 15-7](#listing-15-7) and [Listing 15-6](#listing-15-6) is that here we set
...
returns a reference to the inner data. [Listing 15-10](#listing-15-10) contains an implementation
...
value in a tuple struct. The `main` function in [Listing 15-9](#listing-15-9) that calls `*` on
...
[Listing 15-8](#listing-15-8) as well as the implementation of `Deref` that we added in [Listing 15-10](#listing-15-10). [Listing 15-11](#listing-15-11) shows the definition of a function that has a string slice
parameter.
