...
_المجموعات_ (collections). معظم أنواع البيانات الأخرى تمثل قيمة واحدة محددة، لكن
المجموعات يمكن أن تحتوي على قيم متعددة. على عكس أنواع المصفوفات والصفوف
المدمجة، البيانات التي تشير إليها هذه المجموعات مخزنة على الكومة (heap)، مما
...
- _المتجه_ (vector) يسمح لك بتخزين عدد متغير من القيم بجانب بعضها البعض.
- _السلسلة النصية_ (string) هي مجموعة من الأحرف. لقد ذكرنا نوع `String`
  سابقًا، لكن في هذا الفصل، سنتحدث عنه بعمق.
- _خريطة التجزئة_ (hash map) تسمح لك بربط قيمة بمفتاح محدد. إنها
...
سنناقش كيفية إنشاء وتحديث (update) المتجهات والسلاسل النصية وخرائط التجزئة، بالإضافة إلى
