...
store elements of only one type. We created a workaround in [Listing 8-9](./ch08-01-vectors.html#8-9) where
...
[Listing 17-3](#17-3) shows how to define a trait named `Draw` with one method named
`draw`:
...
<span class="caption" id="17-3">Listing 17-3: Definition of the `Draw` trait</span>

This syntax should look familiar from our discussions on how to define traits
in Chapter 10. Next comes some new syntax: [Listing 17-4](#17-4) defines a struct named
...
<span class="caption" id="17-4">Listing 17-4: Definition of the `Screen` struct with a
`components` field holding a vector of trait objects that implement the `Draw`
trait</span>

On the `Screen` struct, we’ll define a method named `run` that will call the
`draw` method on each of its `components`, as shown in [Listing 17-5](#17-5):
...
<span class="caption" id="17-5">Listing 17-5: A `run` method on `Screen` that calls the
...
<span class="caption" id="17-6">Listing 17-6: An alternate implementation of the `Screen`
...
might have fields for `width`, `height`, and `label`, as shown in [Listing 17-7](#17-7):
...
<span class="caption" id="17-7">Listing 17-7: A `Button` struct that implements the
`Draw` trait</span>
...
`SelectBox` type as well, as shown in [Listing 17-8](#17-8):
...
<span class="caption" id="17-8">Listing 17-8: Another crate using `gui` and implementing
...
components. [Listing 17-9](#17-9) shows this implementation:
...
<span class="caption" id="17-9">Listing 17-9: Using trait objects to store values of
...
in [Listing 17-5](#17-5), `run` doesn’t need to know what the concrete type of each
...
For example, [Listing 17-10](#17-10) shows what happens if we try to create a `Screen`
...
<span class="caption" id="17-10">Listing 17-10: Attempting to use a type that doesn’t
...
extra flexibility in the code that we wrote in [Listing 17-5](#17-5) and were able to
support in [Listing 17-9](#17-9), so it’s a trade-off to consider.
