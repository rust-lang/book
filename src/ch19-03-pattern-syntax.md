...
يتحقق المصرِّف من أن النطاق ليس فارغًا في وقت الترجمة، ولأن الأنواع الوحيدة التي يمكن لـ Rust معرفة ما إذا كان النطاق فارغًا أم لا هي `char` والقيم الرقمية، فإن النطاقات مسموحة فقط مع القيم الرقمية أو قيم `char`.
...
### التفكيك (Destructuring) لتفريق القيم

يمكننا أيضًا استخدام الأنماط لتفكيك (destructuring) الهياكل والتعدادات والصفوف لاستخدام أجزاء مختلفة من هذه القيم. لنستعرض كل قيمة.
...
ومع ذلك، يجب أن يكون استخدام `..` غير غامض. إذا لم يكن واضحًا أي القيم مخصصة للمطابقة وأي يجب تجاهلها، فسيعطينا Rust خطأ. توضح القائمة 19-25 مثالاً على استخدام `..` بشكل غامض، لذلك لن يتم ترجمتها.
...
لا توجد طريقة للتعبير عن الشرط `if x % 2 == 0` داخل نمط، لذلك يمنحنا حارس المطابقة القدرة على التعبير عن هذا المنطق. الجانب السلبي من هذا التعبير الإضافي هو أن المصرِّف لا يحاول التحقق من الشمولية عندما تكون تعبيرات حراس المطابقة متورطة.
...
أنماط Rust مفيدة جدًا في التمييز بين أنواع مختلفة من البيانات. عند استخدامها في تعبيرات `match`, يضمن Rust أن أنماطك تغطي كل قيمة ممكنة، أو لن يتم ترجمة برنامجك. الأنماط في تعليمات `let` ومعاملات الدوال تجعل تلك البنى أكثر فائدة، مما يتيح تفكيك القيم إلى أجزاء أصغر وتعيين تلك الأجزاء إلى متغيرات. يمكننا إنشاء أنماط بسيطة أو معقدة لتناسب احتياجاتنا.
