...
قبل تشغيل برنامج Rust، يجب عليك تجميعه باستخدام مصرِّف Rust عن طريق إدخال أمر `rustc` وتمرير اسم ملف المصدر الخاص بك، مثل هذا:

```console
$ rustc main.rs
```

إذا كانت لديك خلفية في C أو C++، ستلاحظ أن هذا يشبه `gcc` أو `clang`. بعد الترجمة بنجاح، يخرج Rust ملفاً تنفيذياً ثنائياً.
...
إذا كنت أكثر إلماماً بلغة ديناميكية، مثل Ruby أو Python أو JavaScript، فقد لا تكون معتاداً على ترجمة وتشغيل برنامج كخطوات منفصلة. Rust هي لغة _مجمّعة مسبقاً (ahead-of-time compiled)_، مما يعني أنه يمكنك ترجمة برنامج وإعطاء الملف التنفيذي لشخص آخر، ويمكنه تشغيله حتى بدون تثبيت Rust. إذا أعطيت شخصاً ملف _.rb_ أو _.py_ أو _.js_، فإنهم يحتاجون إلى تثبيت تنفيذ Ruby أو Python أو JavaScript (على التوالي). ولكن في تلك اللغات، تحتاج فقط إلى أمر واحد لتجميع وتشغيل برنامجك. كل شيء عبارة عن مقايضة في تصميم اللغة.
