## المستقبلات (Futures) وصيغة Async

العناصر الأساسية للبرمجة غير المتزامنة (asynchronous programming) في Rust هي _المستقبلات_ (futures) وكلمات `async` و `await` الأساسية في Rust.
...
السبب في أن `main` لا يمكن أن تكون محددة بـ `async` هو أن الكود غير المتزامن (asynchronous) يحتاج إلى _بيئة تشغيل_ (runtime): مكتبة Rust تدير تفاصيل تنفيذ الكود غير المتزامن (asynchronous). يمكن لدالة `main` في البرنامج _تهيئة_ بيئة تشغيل، لكنها ليست بيئة تشغيل _بحد ذاتها_. (سنرى المزيد حول سبب ذلك لاحقًا.) كل برنامج Rust ينفذ كود async لديه مكان واحد على الأقل حيث يقوم بإعداد بيئة تشغيل تنفذ المستقبلات.
