## الأخطاء القابلة للاسترداد (Recoverable Errors) مع `Result`
...
`T` و `E` هما معاملات نوع عامة: سنناقش الأنواع العمومية بمزيد من
...
### اختصارات لـ Panic عند الخطأ (Error): `unwrap` و `expect`
...
بالمثل، تتيح لنا دالة `expect` أيضًا اختيار رسالة خطأ (error) `panic!`. استخدام
`expect` بدلاً من `unwrap` وتوفير رسائل خطأ جيدة يمكن أن ينقل نيتك ويجعل
تتبع مصدر الذعر (panic) أسهل. تبدو صياغة `expect` كما يلي:
...
فعله. هذا يُعرف بـ _نشر_ الخطأ ويعطي مزيدًا من التحكم للكود المستدعي،
...
نمط نشر الأخطاء (error propagation) هذا شائع جدًا في Rust لدرجة أن Rust توفر
علامة الاستفهام `?` لجعل هذا أسهل.

#### اختصار لنشر الأخطاء (Error Propagation): معامل `?` (? operator)

القائمة 9-7 توضح تطبيقًا لـ `read_username_from_file` له نفس
الوظيفة مثل القائمة 9-6، لكن هذا التطبيق يستخدم
معامل `?` (? operator).

<Listing number="9-7" file-name="src/main.rs" caption="دالة تُرجع الأخطاء إلى الكود المستدعي باستخدام معامل `?` (? operator)">
...
يفعله معامل `?` (? operator): قيم الخطأ (error) التي يُستدعى عليها معامل `?` تمر عبر
دالة `from`، المعرّفة في trait `From` في المكتبة القياسية،
والتي تُستخدم لتحويل قيم من نوع إلى آخر. عندما يستدعي معامل `?` دالة
`from`، يُحوّل نوع الخطأ (error) المُستقبل إلى نوع الخطأ (error) المعرّف في نوع
الإرجاع للدالة الحالية. هذا مفيد عندما تُرجع دالة نوع خطأ (error) واحد
...
<Listing number="9-8" file-name="src/main.rs" caption="ربط استدعاءات الدوال بعد معامل `?` (? operator)">
...
#### أين يمكن استخدام معامل `?` (? operator)

يمكن استخدام معامل `?` (? operator) فقط في الدوال التي نوع إرجاعها متوافق
مع القيمة التي يُستخدم عليها `?`. هذا لأن معامل `?` معرّف
...
في القائمة 9-10، لننظر إلى الخطأ (error) الذي سنحصل عليه إذا استخدمنا معامل `?` (? operator)
في دالة `main` بنوع إرجاع غير متوافق مع نوع
القيمة التي نستخدم `?` عليها.

<Listing number="9-10" file-name="src/main.rs" caption="محاولة استخدام معامل `?` (? operator) في دالة `main` التي تُرجع `()` لن تُترجم.">
...
هذا الكود يفتح ملفًا، وقد يفشل ذلك. يتبع معامل `?` (? operator) قيمة `Result`
المُرجعة من `File::open`، لكن دالة `main` هذه لها نوع إرجاع
`()`، وليس `Result`. عندما نترجم هذا الكود، نحصل على رسالة الخطأ (error)
التالية:
...
يشير هذا الخطأ (error) إلى أننا مسموح لنا فقط باستخدام معامل `?` (? operator) في
دالة تُرجع `Result` أو `Option` أو نوع آخر ينفذ
`FromResidual`.

لإصلاح الخطأ (error)، لديك خياران. الخيار الأول هو تغيير نوع الإرجاع
لدالتك ليكون متوافقًا مع القيمة التي تستخدم معامل `?` (? operator) عليها
طالما ليس لديك قيود تمنع ذلك. الخيار الآخر هو
استخدام `match` أو إحدى دوال `Result<T, E>` للتعامل مع `Result<T, E>`
بالطريقة المناسبة.

رسالة الخطأ (error) ذكرت أيضًا أن `?` يمكن استخدامه مع قيم `Option<T>`
أيضًا. كما هو الحال مع استخدام `?` على `Result`، يمكنك فقط استخدام `?` على `Option` في
دالة تُرجع `Option`. سلوك معامل `?` (? operator) عند استدعائه على
...
<Listing number="9-11" caption="استخدام معامل `?` (? operator) على قيمة `Option<T>`">
...
الأول، سيتم إرجاعه في متغير `Some`. يعطينا معامل `?` (? operator) في الوسط
طريقة موجزة للتعبير عن هذا المنطق، مما يسمح لنا بتنفيذ
الدالة في سطر واحد. إذا لم نتمكن من استخدام معامل `?` (? operator) على `Option`، كان سيتعين
علينا تنفيذ هذا المنطق باستخدام المزيد من استدعاءات الدوال أو تعبير `match`.

لاحظ أنه يمكنك استخدام معامل `?` (? operator) على `Result` في دالة تُرجع
`Result`، ويمكنك استخدام معامل `?` (? operator) على `Option` في دالة
تُرجع `Option`، لكن لا يمكنك المزج والمطابقة. لن يحول معامل `?` (? operator)
...
<Listing number="9-12" file-name="src/main.rs" caption="تغيير `main` لإرجاع `Result<(), E>` يسمح باستخدام معامل `?` (? operator) على قيم `Result`.">
