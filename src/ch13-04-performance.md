...
## الأداء في الحلقات مقابل المُكرِّرات (Iterators)

لتحديد ما إذا كان يجب استخدام الحلقات أو المُكرِّرات (iterators)، تحتاج إلى معرفة أي تنفيذ أسرع: نسخة دالة `search` مع حلقة `for` صريحة أو النسخة مع المُكرِّرات.
...
الإغلاقات (closures) والمُكرِّرات (iterators) هي ميزات Rust مستوحاة من أفكار لغات البرمجة الوظيفية (functional programming). تساهم في قدرة Rust على التعبير بوضوح عن الأفكار عالية المستوى بأداء منخفض المستوى. تنفيذات الإغلاقات والمُكرِّرات بحيث لا يتأثر أداء وقت التشغيل. هذا جزء من هدف Rust للسعي لتوفير تجريدات ذات تكلفة صفرية.
