...
<span class="caption">Listing 8-12: Creating a new, empty `String`</span>

This line creates a new empty string called `s`, which we can then load data
into. Often, we’ll have some initial data that we want to start the string
with. For that, we use the `to_string` method, which is available on any type
that implements the `Display` trait, as string literals do. Listing 8-13 shows
two examples:
...
<span class="caption">Listing 8-13: Using the `to_string` method to create a
`String` from a string literal</span>

This code creates a string containing `initial contents`.

We can also use the function `String::from` to create a `String` from a string
literal. The code in Listing 8-14 is equivalent to the code from Listing 8-13
that uses `to_string`:
...
<span class="caption">Listing 8-14: Using the `String::from` function to create
...
data in them, as shown in Listing 8-15:
...
<span class="caption">Listing 8-15: Storing greetings in different languages in
strings</span>
...
<span class="caption">Listing 8-16: Appending a string slice to a `String`
using the `push_str` method</span>

After these two lines, `s` will contain `foobar`. The `push_str` method takes a
string slice because we don’t necessarily want to take ownership of the
parameter. For example, the code in Listing 8-17 shows that it would be
...
<span class="caption">Listing 8-17: Using a string slice after appending its
...
`String`. Listing 8-18 shows code that adds the letter l to a `String` using
the `push` method:
...
<span class="caption">Listing 8-18: Adding one character to a `String` value
using `push`</span>
...
operator, as shown in Listing 8-19:
...
<span class="caption">Listing 8-19: Using the `+` operator to combine two
...
the second parameter to `add`. So why does Listing 8-19 compile?
...
because `self` does *not* have an `&`. This means `s1` in Listing 8-19 will be
...
get an error. Consider the invalid code in Listing 8-20:
...
<span class="caption">Listing 8-20: Attempting to use indexing syntax with a
String</span>
...
encoded UTF-8 example strings from Listing 8-15. First, this one:
