...
[Listing 8-5][Listing-8-5] shows the method of accessing a value in a vector with
indexing syntax:
...
[Listing 8-6][Listing-8-6] shows the method of accessing a value in a vector, with
the `get` method:
...
shown in [Listing 8-7][Listing-8-7]:
...
scope. That rule applies in [Listing 8-8][Listing-8-8], where we hold an immutable reference to
...
The code in [Listing 8-8][Listing-8-8] might look like it should work: why should a reference
...
all of the elements rather than use indexes to access one at a time. [Listing 8-9][Listing-8-9]
...
in order to make changes to all the elements. The `for` loop in [Listing 8-10][Listing-8-10]
...
ultimately, holds different types. We’ve demonstrated this in [Listing 8-11][Listing-8-11]:
...
[Listing-8-11]: ch08-01-vectors.html#Listing-8-11
