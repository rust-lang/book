...
We can also add a custom message to be printed with the failure message as an
optional second argument to the `assert!` macro and an optional third argument
to the `assert_eq!` and `assert_ne!` macros. Custom messages are useful in
order to document what an assertion means, so that when the test fails, we have
a better idea of what the problem is with the code.

TODO: real-ish example of custom messages
