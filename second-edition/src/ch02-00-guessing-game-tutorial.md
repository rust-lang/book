...
структуры имеющий тип `String`. Этот тип данных входит в стандартную библиотеку.
...
Результатом вызова Функции `new` является новая, пустая `String`. С `new`,
я думаю, вы ещё неоднократно столкнётесь изучая код стандартной библиотеки, т.к.
это общее имя функции, которую создаёт экземпляр определённого типа.
...
которой мы узнали из работы с данной строкой кода, является то, что переменные
...
Сначала изменим код так, чтобы некоторые его части можно было бы закоментировать `mut`,
...
Помимо этого эта фукнция возвращает значение [`io::Result`][ioresult]<!-- ignore -->.
...
Основной целью `Result`-типов является понятное значение для последующего анализа информации
...
Программа не обрабатывает возможные ошибки. Правильным решением будет написать 
обработчик ошибки. Вызов фукции `expect` — наиболее простое решение. 
Боле подробно об обработки ошибок мы познакомится в главе № 9.

### Особенности работы с макросом `println!`

Текстовое содержание входных данных макроса `println!` может содержать специальные
блоки, которые могут быть заполнены данными, передаваемыми следующими за строкой
параметрами:
...
А хотите уже почувтвовать себя хакером Rust?! :-) Придумайте задачку на основе имеющихся у
Вас знаний о переменных и макросе `println!`, чтобы компилятор долго, долго подумал
прежде чем выполнить код программы.

Вот вам код для ваших идей:
...
Пожалуйста, придумайте как выполнение этой строчки скода сделать медленным.

Если на вашем компьютере ваш код будет компилироваться уж очень долго — решение —
сочетание клавиш останавливающих выполнение программы — (`Ctrl-C`).
Придумывайте, фантазируйте с Rust! :-)
...
Пример компиляции и выполнения кода вашей программы в строке терминала:
...
того, как мы добавили информацию о зависимости нашего проекта от пакета `rand`</span>
...
сторонних проектом. Он предлагает сравнительно простое решение. Это своеобрзный
...
При повторной сборке проекта, Cargo, основываясь на данных файла Cargo.lock.
Проект изменит данный от зависимостях только лишь тогда, когда будет явно вызвана
...
1. При этом игнорируются данные файла *Cargo.lock*. Производится поик последний версий пакетов, описанных в файле *Cargo.toml*
...
Всё эти условности — следствия семантической теории версий пакетов. Пожалуйста,
...
Благодаря активном использованию пакетов создания ваших приложений становится проще,
...
Преступим к *использованию* функционала пакета `rand`. Когда необходимые пакеты
...
Кроме декларирования номерений, мы написали код, который использует вышеописанный
...
чисел, ссылку на которого мы используем в цепочке кода. Мы вызываем его метод
...
отрезка. Псевдослучайное число большое или равно 1 и меньше 101.
...
Вас большую уверенности. Для удобства программистов Cargo  предоставляет удобную
возможность получения документации с помощь команды `cargo doc --open`. Кроме того, 
что создаётся локальная версия документации, она становиться доступна в виде локального 
сайта. Если вам необходим доступ и информации о других каких-либо возможностях пакета `rand` 
—  просто введите команду `cargo doc --open` и щелкните по левому меню слева (где будут 
находится ссылки на используемые вашем проектом и его внешними зависимостями пакетами). 
Конечно, пока не очень понятно, что обозначают конструкции кода, но путешествие по страницам 
данной документации даст общее представление о возможностях утилит, которые в вашей дальнейшей 
работе с Rust будут весьма полезны.
...
анализа введёных данных (сравнить то что ввёл игрок с тем, что сгенерировал функционал
пакета `rand`):
(Listing 2-4):
...
благодаря которому выбирается действие, которое выполниться в зависимости от переданного
значения.
...
Теоретически такой код должен работать. Проверим это на практике:
...
разных типов, чтобы не создавать новые. Такая вот интересная оптимизации на уровне
...
вызван далее удалил из пробелы с начала и с конца введённой строки. Также этот метод
удаляет все непечатаемые символы — такие, как знак табуляции, перенос на новую строку,
возврат каретки.

Метод [`parse`][parse]<!-- ignore --> как раз конвертирует текст в число. Т.к.
данный метод позволяет получать разные числовые типы, мы должны явно указать, какой
тип мы хотим получить. Задание типа переменной `let guess: u32` отлично решение для
этой задачи. Тип `u32` максимально близок к числам заданного отрезка. Вы познакомитесь
...
```
Введённое слово `quit`, конечно же остановило программу, но на его месте могло бы
быть любое иное слова и даже знак препинания. Для корректной работы программ нужны
...
программы, если игра выйграна. Для этого добавить в наш код ключевое
слово языка программирования Runst — `break`:
...
расскажут вам более подробно об всем этом более подробно. Глава 3 рассказывает
об общеязыковых концепциях Rust — переменные, типы данных, функции. Разбираются
примеры их использования. Глава 4 повествует о владении. Это одна из отличительный
