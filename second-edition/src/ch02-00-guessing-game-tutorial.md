...
메소드, 연관함수(assiciated functions), 외부 크래이트(external crates) 등의 활용 방법을
배울 수 있습니다. 이런 개념들은 다음 장들에서 더 자세히 다뤄질 것입니다. 이번 장에서는
여러분이 직접 기초적인 내용을 실습합니다.

우리는 고전적인 입문자용 프로그래밍 문제인 추리 게임을 구현해 보려 합니다. 이
프로그램은 1~100 사이의 임의의 정수를 생성합니다. 다음으로 플레이어가 프로그램에 추측한
정수를 입력합니다. 프로그램은 입력받은 추측값이 정답보다 높거나 낮은지를 알려줍니다.
추측값이 정답이라면 축하 메세지를 보여주고 종료됩니다.
...
검증합니다. 첫 시작으로 플레이어에게 추측값을 입력받을 수 있게 할 것입니다.
...
<span class="caption">Listing 2-1: 사용자에게 받은 추즉값을 그대로 출력하는 코드</span>
...
이 라인이 *변수* 를 생성하는 `let`문임을 명심하세요. 다음 코드도 변수를 선언하는
예시입니다.
...
입력 핸들에서 `.read_line(&mut guess)`를 호출합니다. 또한 `read_line`에 `&mut guess`
를 인자로 하나 넘깁니다.
...
`.foo()` 다음에 메소드를 호출할 경우 긴 라인을 나누기 위해 다음 줄과 여백을 넣는 것이
...
여러 `Result`는 에러 처리를 위한 정보를 표현하기 위해 사용됩니다. `Result` 타입의
...
지금까지 작성한 코드에서 닫는 대괄호 말고도 살펴봐야 하는 코드가 하나 더 있습니다. 내용은
아래와 같습니다.
...
다음으로 사용자가 추측하기 위한 비밀번호를 생성해야 합니다. 게임을 다시
하더라도 재미있도록 비밀번호는 매번 달라야 합니다. 게임이 너무 어렵지 않도록
1에서 100 사이의 임의의 수를 사용합시다. 러스트는 아직 표준 라이브러리에
임의의 값을 생성하는 기능이 없습니다. 하지만 러스트 팀에서는 
[`rand` 크래이트][randcrate]를 제공합니다.

[randcrate]: https://crates.io/crates/rand

### 크래이트(Crate)를 사용하여 더 많은 기능 가져오기

*크래이트*는 러스트 코드의 묶음(package)임을 기억하세요. 우리가 만들고 있는
프로젝트는 실행이 가능한 *binary crate* 입니다. `rand` crate는 다른
프로그램에서 사용되기 위한 용도인 *library crate* 입니다.

Cargo에서 외부 크래이트의 활용이 정말 멋진 부분입니다. `rand`를 사용하는
코드를 작성하기 전에 *Cargo.toml* 을 수정하여 `rand` 크래이트를 의존 리스트에
추가해야 합니다. 파일을 열고 Cargo가 여러분을 위해 생성한 `[dependencies]`
부분에 아래처럼 내용을 추가하세요.
...
*Cargo.toml* 에서는 헤더 다음의 내용은 다음 헤더가 나타낼 때까지 그 헤더에
포함됩니다.  [dependencies] 절을 통해 Cargo에게 여러분의 프로젝트가
어떤 외부 크래이트에 의존하고 있는지와 어떤 버전이 필요한가를 알려줄
수 있습니다. 지금의 경우 우리는 `rand` 크래이트의 유의적 버전인 `0.3.14`을
명시했습니다. Cargo는 버전을 명시하는 표준에 해당하는
...
<span class="caption">Listing 2-2: rand 크래이트를 의존성으로 추가한 후
`cargo build` 를 실행한 결과</span>

여러분은 다른 버전명이나 라인의 순서가 다르게 보일 수 있습니다. 버전명이
다르더라도 SemVer 덕분에 현재 코드와 호환될 것입니다.

이제 우리는 외부 의존성을 가지게 되었고, Cargo는 [Crates.io][cratesio]의 복사본인
*레지스트리(registry)*에서 모든 것들을 가져옵니다. Cartes.io는 러스트의 
생태계의 개발자들이 다른 사람들도 이용할 수 있도록 러스트 오픈소스를
공개하는 곳입니다.

[cratesio]: https://crates.io

레지스트리를 업데이트하고 난 후 Cargo는 `[dependencies]` 절을 확인하고
아직 여러분이 가지고 있지 않은 것들을 다운 받습니다. 이 경우 우리는 `rand`만
의존한다고 명시했지만 `rand`는 `libc`에 의존하기 때문에 `libc`도 다운 받습니다.
이것들을 다운받은 후 러스트는 그것들을 컴파일하고 의존성이 해결된 상태에서
프로젝트를 컴파일합니다.

만약 아무것도 변경하지 않고 `cargo build`를 실행한다면 어떠한 결과도 얻지
못할 것입니다. Cargo는 이미 의존 패키지들을 다운받고 컴파일했음을 알고 있고
여러분이  *Cargo.toml*를 변경하지 않은 것을 알고 있습니다. 또한 Cargo는 코드가
변경되지 않은 것도 알고 있음으로 코드도 다시 컴파일하지 않습니다. 아무것도 할
일이 없기에 그냥 종료될 뿐입니다. 만약 여러분이 *src/main.rs*파일을 열어 사소한
변경을 하고 저장한 후 다시 빌드를 한다면 한 라인이 출력됨을 확인할 수 있습니다.
...
이 라인은 Cargo가 *src/main.rs*의 작은 변경을 반영하여 빌드를 업데이트함을
보여줍니다. 의존 패키지가 변경되지 않았으므로 Cargo는 이미 다운받고 컴파일된
것들을 재사용 가능함을 알고 있습니다. 따라서 Cargo는 여러분의 코드에 해당하는
부분만을 재빌드합니다.
...
전까지는 여러분이 명시한 의존 패키지만을 사용합니다. 예로 `rand` 크래이트의 다음
버전인 `v0.3.15`에서 중요한 결함이 고쳐졌지만 당신의 코드를 망치는 변겅점(regression)
이 있다면 어떻게 될까요?
...
#### 크래이트를 새로운 버전으로 업그레이드하기

만약 당신이 *정말* 크래이트를 업데이트하고 싶은 경우를 위해 Cargo는 `update` 명령어를
제공합니다. 이것은
...
만약 `rand` 크래이트가 새로운 두 개의 버전인 `0.3.15`와 `0.4.0`을 릴리즈했다면
...
크래이트의 버전이 `0.3.15`임을 확인할 수 있습니다.
...
다음번에 여러분이 `cargo build`를 실행하면 Cargo는 가용 가능한 크래이트들의
레지스트리를 업데이트할 것이고 여러분의 `rand`요구사항을 새롭게 명시한 버전에
따라 재계산할 것입니다.

[Cargo][doccargo]<!-- ignore -->와 [그의 생태계][doccratesio]<!-- ignore -->에 대해 더 많은 것들은
14장에서 다뤄지지만 지금 당장은 이것들이 여러분이 알아야 할 내용입니다.
Cargo는 라이브러리의 재사용을 쉽게 하여 러스트 사용자들이 많은 패키지들과
결합된 더 작은 프로젝트들을 작성할 수 있도록 도와줍니다.
...
<span class="caption">Listing 2-3: 임의의 숫자를 생성하기 위해 필요한
코드</span>

우리는 `extern crate rand;`을 추가하여 러스트에게 우리가 외부에 의존하는
크래이트가 있음을 알립니다. 이 라인은 `use rand`으로도 표기할 수 있으며
...
크래이트에서 어떤 trait를 사용하고 어떤 함수나 메소드들을 호출하는 것을
아는 것은 단순히 *아는 것*이 아닙니다. 각각의 크래이트의 문서에서 사용 방법을
제공합니다. 또다른 멋진 특성은 `cargo doc --open` 명령어로써 로컬에서
여러분의 모든 의존 패키지들이 제공하는 문서들을 빌드해서 브라우저에 표시해
줍니다. 만약 `rand` 크래이트의 다른 기능들에 흥미가 있다면
...
최종적으로 우리는 추측값을 정수형으로 비교하기 위해 입력으로 받은 `String`을 정수로 바꾸고
...
포함되어 있다면 정수로 바꿀 방법이 없습니다. "Result 타입으로 가능성 있는
실패 처리하기"에서 `read_line`와 비슷하게 `parse` 메소드는 실패할 경우를 위해 
`Result` 타입을 결과로 돌려 줍니다. 만약 `parse` 메소드가 문자열에서 정수로
파싱을 실패하여 `Err` `Result` variant를 돌려준다면 `expect` 호출은 게임을 멈추고
우리가 명시한 메세지를 출력합니다. 만약 `parse` 메소드가 성공적으로 문자열을
정수로 바꾸었다면 `Result`의 `Ok` variant를 돌려 받으므로 `expect`에서 `Ok`에서
얻고 싶었던 값을 결과로 받게 됩니다.
...
좋습니다! 추측값 앞에 빈칸을 넣더라도 프로그램은 추측값이 76임을 파악 했습니다.
추측값이 맞을 때나 너무 클 경우, 혹은 너무 작은 경우 등 여러 종류의 입력값으로
여러 시나리오를 검증해 봅시다.

우리는 게임의 대부분이 동작하도록 처리 했지만 사용자는 한 번의 추측만 가능합니다.
반복문을 추가하여 변경해 봅시다!

## 반복문을 이용하여 여러 번의 추측 허용
...
또 다른 추측값을 요청하도록 합니다. 효율적으로 프로그램은 `parse`에서 가능한
모든 에러를 무시합니다.
...
이 프로젝트는 `let`, `match`, 메소드, 연관함수, 외부 크래이트 사용과 같은 많은
