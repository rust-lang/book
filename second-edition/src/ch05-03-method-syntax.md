...
> аналогичны.
>
> Rust не имеет такого эквивалента (оператора `->`), наоборот, в Rust функционал,
...
> добавляет `&`, `&mut`, or `*` так  чтобы `object` имеет соответствующие опции
...
Тот факт, что описание владения происходит неявно, делать код программы более компактным.
