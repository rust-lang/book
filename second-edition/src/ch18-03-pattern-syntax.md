...
여러분은 `match` 표현 내에서 *or* 을 뜻하는 `|` 구문을 이용해 다중 패턴과
...
리터럴 값을 이용해 해체할 수도 있습니다. 우린 이를 이용해
일부 필드가 특정 값을 만족하는지 를 나머지 필드를 해체하면서
테스트할 수 있습니다.
...
패턴을 이용한 해체는 구조체의 각 필드 값 같은 값의 일부를 서로 별도로
사용하는 편리한 방법입니다.
...
우린 `_` 언더스코어 와일드카드 패턴을 어떤 값과도 매치되지만,
값으로 바인드(bind) 하진 않는 것으로 사용해왔습니다. `_` 언더스코어 패턴은
`match` 의 마지막 갈래 표현에서 특히 유용하지만, 이는 Listing 18-17 처럼
...
여전히 값이 바인드되기 때문에 해당 값의 소유권을 가져간다</span>
...
<span class="caption">Listing 18-22: 값이 바인드되지 않는
언더스코어 사용하기</span>
...
갖습니다. `match` 표현에선 `x` 좌표만 사용하고 `y` 와 `z` 필드의 값은
무시합니다:
...
이것이 있을 경우 패턴 매칭과 이것이 모두 만족되어야 해당 갈래가 선택됩니다.
