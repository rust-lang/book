...
Мы будем симулировать работу алгоритма расчета параметров с помощью функции
...
Остальные изменения в коде будут сделаны в этой функции:
...
необходим символ `;`. Обратите внимание, что после `num` (последняя строчка блока 
анонимной функции) нет `;`. Это означает, что переменная будет возращена функцией.
