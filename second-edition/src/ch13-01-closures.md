...
클로저를 정의하면서, `if` 블럭에 있는 코드를 그 코드를 실행하기 위해 클로저를 호출
하고 결과값을 얻도록 변경할 수 있습니다.우리는 함수를 호출하는 것 처럼 클로저를 호출
합니다: 리스트 13-6 에 보여지는 것처럼, 클로저 정의를 갖고 있는 변수명을 쓰고
다음엔 사용할 인자값을 포함하는 괄호가 따라 옵니다:
...
`if` 블럭에서 클로저를 두번 호출 합니다, 비용이 큰 코드를 두번 호출하고 사용자가
