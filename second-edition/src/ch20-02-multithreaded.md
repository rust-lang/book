...
이 코드는 `receiver` 을 여러개의 `Worker` 객체에 전송하도록 시도하는데, 이는 작동하지 않습니다.
...
스레드들은 *pool* 이 존재하는 한 계속 실행 될 겁니다.
...
이 오류는 상당히 수수께끼스럽습니다. `Box<T>` (`Job` 가 가르키는 그것)
...
우리는 이제 `Job` 타입 별명이 새로운 특성 인 `FnBox` 를 구현하는 `Box` 가 될
필요가 있습니다. 이는 클로저를 직접 호출하는 대신 `Job` 값을 얻을 때 `Worker`
에서 `call_box` 를 사용할 수 있게 해줍니다. 어떤 `FnOnce()` 클로저에 대한
`FnBox` 특성을 구현한다는 것은 채널을 보내고있는 실제 값에 대해서는 아무것도
