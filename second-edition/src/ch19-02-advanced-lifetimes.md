...
참조자의 라이프타임이 어떻게 연관되는지를 알려주기 위하여 라이프타임 파라미터를
이용해서 참조자를 명시하는 방법을 배웠습니다. 여러분은 모든 참조자가 라이프타임을
갖지만, 거의 대부분의 경우 러스트가 어떻게 이 라이프타임을 생략시켜주는지도 봤습니다.
이제 우리는 아직 다루지 못했던 라이프타임의 세가지 고급 기능을 살펴볼 것입니다:

* 라이프타임 서브타이핑 (subtyping): 한 라이프타임이 다른 라이프타임보다 오래 사는 것을 보장하기
* 라이프타임 바운드: 제네릭 타임을 가리키는 참조자를 위한 라이프타임 명시하기
...
않고 있습니다. 하지만, 파싱 로식의 어딘가에서 잘못된 입력 부분을 참조하는
에러를 반환하는 것으로 유효하지 않은 입력을 다룰 가능성은 매우 높습니다;
이 참조자는 라이프타임에 대하여 코드를 흥미롭게 만들어주는 것입니다.
우리 파서의 로직이 첫번째 바이트 이후의 입력은 유효하지 않다고 판단했다
...
필요가 있습니다. 이를 하기 위한 가장 직관적인 방법은 Listing 19-13에서
...
반면, 예제에서 선언된 이 3개의 라이프타임 파라미터는 모두 연관되어있지 않습니다.
...
이 코드는 잘 컴파일 됩니다. 이 코드는 러스트에게 `Parser`가 라이프타임이 `'a`인
...
그러나 이 둘 모두 함수의 전체 라이프타임을 위해 살아야 할 필요가 있습니다.
...
인스턴스는 함수 끝을 벗어나 살수 없을 것이고 (일시적인 객체입니다), `context`는
...
따라서, 이것은 정말로 `parse_context`가 조심스레 다루고자 하는 스트링
슬라이스의 라이프타임인 것입니다. 우리는 `Context` 내의 스트링 슬라이스와
...
문제를 완전히 해결하지는 않겠지만, 이것이 시작점이라는 점을 주목하세요. 이 수정이 컴파일
시도를 했을때 왜 충분치 않은지 살펴보겠습니다.
...
문법이 자주 필요치는 않겠지만, 특정한 상황에서 여러분은 여러분이
참조해야하는 무언가를 참조할 방법을 알아야 할 것입니다.
...
이 코드는 이제 컴파일되는데 이는 `T: 'a` 문법이 `T`가 어떤 타입이든 될 수 있지만,
