...
did not contain valid UTF-8", "multiple input filenames provided", ..)) — будет напечатана строка `Hello, Rust world! Привет, Мир!`.
...
Этот текст определяет *Rust-функцию*. Функция `main` — особенная. Это так называемая точка
...
В списке присутствуют три файла: файл с исходным кодом программы *.rs*, файл базы данных программы *.pdb* и бинарный
...
Если бы в исходном коде *main.rs* в входных параметрах макроса `println` был введён текст
...
Компиляция простых программ с помощью `rustc` - это замечательно, но если ваш проект 
становится всё больше и больше, то вам понадобятся средства управления всеми компонентами.
Удобные возможности для обеспечения цикла командной разработки также не были бы лишними. 
Далее, мы представим вам программу `cargo`, которая поможет создать удобную среду разработки.
...
если вы пролжаете работать с тоже терминальном окне предыдущего примера.

Далее введём следующие команды в несколько последовательных этапов:
...
```shell
$ cd .. & cargo new hello_cargo2 --bin --vcs none & cd hello_cargo2
```
...
2) В приложении подключайтесь к своей учётной записи.
...
Для Windows, кроме того, можно создать *.bat-файл hello_cargo.bat в корне проекта со следующим содержанием:
```shell
%~dp0target\debug\hello_cargo
```
Этот скрипт запускается из командной строки из корневой папке проекта (для удобства), где `~dp0` - полный путь 
к папке, из которого он запускается:
...
[официальным руководством]: https://doc.rust-lang.org/cargo/guide/
