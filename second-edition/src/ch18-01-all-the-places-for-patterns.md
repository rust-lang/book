...
그래서 `match`의 마지막 갈래에 종종 쓰입니다. `_`패턴은 명시 하지 않은 값들을 무시할
때 유용합니다. 이 특별한 패턴은 뒤의 "패턴에서 값 무시하기" 절에서 더 자세히 다룰
것입니다.
...
예제 18-1은 배경 색을 정하기 위해 여러 조건을 체크하는 코드입니다.
실제의 유저 입력을 재현하기 위해 예제에서는 하드 코딩된 값들을 사용합니다.
...
하지만 이미 여러 곳에서 패턴을 사용하고 있었습니다. 대표적으로 `let`구문이 있습니다.
...
다룰 `_`나 `..`를 사용할 수 있습니다. 반대로 문제가 패턴 쪽의 변수가 너무 많아서
생겼다면 패턴의 변수 일부를 없에는 것으로 튜플의 원소의 수와 변수의 원소의 수를 맞추어
타입을 맞춰주면 됩니다.
