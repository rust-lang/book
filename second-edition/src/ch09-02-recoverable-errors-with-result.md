...
on the value `File::open` returned. [Listing 9-4][Listing-9-4] shows one way to handle the
...
The code in [Listing 9-4][Listing-9-4] will `panic!` no matter the reason that `File::open`
...
[Listing 9-4][Listing-9-4]. Look at Listing 9-5, which adds another arm to the `match`:
...
The one difference between the `match` expression from [Listing 9-6][Listing-9-6] and what the
...
defined in [Listing 9-6][Listing-9-6]. The part of the `match` that requires a return type of
